...
# Nitrogen data: all yrs, by species - ignore the single boxplots where there should be 1 per yr (not sure why this is happening)
```{r}
ggplot(cn, aes(x = year, y = nitrogen, fill = state)) +
        geom_boxplot(color = "black", outlier.shape = NA) +
        geom_jitter(shape=16, position=position_jitterdodge(), alpha = 0.6, aes(colour = state)) +
        facet_grid(.~species) +

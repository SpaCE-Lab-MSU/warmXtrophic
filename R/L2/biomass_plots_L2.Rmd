...
# plot level KBS
kbs_bio_dot <- ggplot(biomass_sum_k3, aes(x = state, y = average_weight, shape = insecticide)) +
        geom_pointrange(aes(ymin=average_weight-se, ymax=average_weight+se), color="black",linetype="solid",size=1,position=position_dodge(0.2)) +
        ...
        theme(plot.title = element_text(size=20),
              ...
              axis.text.y = element_text(size=17))
# plot level UMBS
umbs_bio_dot <- ggplot(biomass_sum_u3, aes(x = state, y = average_weight, shape = insecticide)) +
        geom_pointrange(aes(ymin=average_weight-se, ymax=average_weight+se), color="black",linetype="solid",size=1,position=position_dodge(0.2)) +
        scale_shape_manual(name="Treatment",
                           values = c(1, 19),
                           labels=c("Herbivory","Reduced Herbivory")) +
...
png("biomass_plots_dots_L2_barplot.png", units="in", width=8, height=4.5, res=300)
...
kbs_spp <- ggplot(biomass_spp_k, aes(x = state, y = avg_weight, fill = state)) +
...
biomass_spp_u <- umbs_biomass_live2 %>%
        ...
        summarize(avg_weight = mean(weight_g, na.rm = TRUE),
                  se = std.error(weight_g, na.rm = TRUE))
umbs_spp <- ggplot(biomass_spp_u, aes(x = state, y = avg_weight, fill = state)) +
        facet_wrap(~species, ncol=4, scales = "free") +
        geom_bar(position = "dodge", stat = "identity", col = "black") +
        geom_errorbar(aes(ymin = avg_weight - se, ymax = avg_weight + se), width = 0.2,
                      position = position_dodge(0.9)) +
...
png("biomass_plots_spp_L2_barplot.png", units="in", width=8, height=6, res=300)
annotate_figure(biomass_spp_comb,
                left = text_grob("Plant Biomass (g)", color = "black", rot = 90),

...
kbs_biomass_only <- kbs_biomass %>%
        dplyr::select(-cover) %>%
        drop_na(weight_g)
umbs_biomass_only <- umbs_biomass %>%
        dplyr::select(-cover) %>%
        drop_na(weight_g)
...
```{r}
# only 2025 data
kbs_biomass_live_25 <- kbs_biomass_live %>% dplyr::filter(year == "2025")
umbs_biomass_live_25 <- umbs_biomass_live %>% dplyr::filter(year == "2025")

# summarizing data
biomass_sum_k_25 <- kbs_biomass_live_25 %>%
        group_by(plot, state) %>%
        summarize(avg_weight = sum(weight_g, na.rm = TRUE))
biomass_sum_k1_25 <- biomass_sum_k_25 %>%
        group_by(state) %>%
        summarize(average_weight = mean(avg_weight, na.rm = TRUE),
                  se = std.error(avg_weight, na.rm = TRUE))
biomass_sum_u_25 <- umbs_biomass_live_25 %>%
        group_by(plot, state) %>%
        summarize(avg_weight = sum(weight_g, na.rm = TRUE))
biomass_sum_u1_25 <- biomass_sum_u_25 %>%
        group_by(state) %>%
        summarize(average_weight = mean(avg_weight, na.rm = TRUE),
                  se = std.error(avg_weight, na.rm = TRUE))

# plot level KBS
kbs_bio_dot_25 <- ggplot(biomass_sum_k1_25, aes(x = state, y = average_weight, fill = state)) +
        geom_pointrange(aes(ymin=average_weight-se, ymax=average_weight+se), 
                        pch=21, size=1, position=position_dodge(0.2)) +
        scale_fill_manual(name="Treatment",
...
# plot level KBS
umbs_bio_dot_25 <- ggplot(biomass_sum_u1_25, aes(x = state, y = average_weight, fill = state)) +
        geom_pointrange(aes(ymin=average_weight-se, ymax=average_weight+se), 
                        pch=21, size=1, position=position_dodge(0.2)) +
        scale_fill_manual(name="Treatment",
                          values = c("#a6bddb", "#fb6a4a")) +
...
biomass_comb_dot <- ggpubr::ggarrange(kbs_bio_dot_25, umbs_bio_dot_25,
                                     ncol = 2, legend="none")
png("biomass_plots_dots_L2.png", units="in", width=8, height=4.5, res=300)
annotate_figure(biomass_comb_dot,
                left = text_grob(bquote("Biomass " ('g/0.20m'^2)), color = "black", rot = 90, size=18),
                bottom = text_grob("Treatment" , color = "black", size=18))
dev.off()
```
...
# summarizing data
litter_sum_k <- kbs_litter %>%
        group_by(plot, state) %>%
        summarize(avg_weight = sum(weight_g, na.rm = TRUE))
litter_sum_k1 <- litter_sum_k %>%
        group_by(state) %>%
        summarize(average_weight = mean(avg_weight, na.rm = TRUE),
                  se = std.error(avg_weight, na.rm = TRUE))

litter_sum_u <- umbs_litter %>%
        group_by(plot, state) %>%
        summarize(avg_weight = sum(weight_g, na.rm = TRUE))
litter_sum_u1 <- litter_sum_u %>%
        group_by(state) %>%
        summarize(average_weight = mean(avg_weight, na.rm = TRUE),
                  se = std.error(avg_weight, na.rm = TRUE))

# plot level KBS
kbs_litter_dot <- ggplot(litter_sum_k1, aes(x = state, y = average_weight, fill = state)) +
        geom_pointrange(aes(ymin=average_weight-se, ymax=average_weight+se), 
                        pch=21, size=1, position=position_dodge(0.2)) +
        scale_fill_manual(name="Treatment",
                          values = c("#a6bddb", "#fb6a4a")) +
...
# plot level UMBS
umbs_litter_dot <- ggplot(litter_sum_u1, aes(x = state, y = average_weight, fill = state)) +
        geom_pointrange(aes(ymin=average_weight-se, ymax=average_weight+se), 
                        pch=21, size=1, position=position_dodge(0.2)) +
        scale_fill_manual(name="Treatment",
                          values = c("#a6bddb", "#fb6a4a")) +
...
litter_comb_dot <- ggpubr::ggarrange(kbs_litter_dot, umbs_litter_dot,
                                     ncol = 2, legend="none")
png("litter_plots_dots_L2.png", units="in", width=8, height=4.5, res=300)
annotate_figure(litter_comb_dot,
                left = text_grob(bquote("Surface Litter Biomass " ('g/0.20m'^2)), color = "black", rot = 90, size=18),
                bottom = text_grob("Treatment" , color = "black", size=18))
dev.off()
```

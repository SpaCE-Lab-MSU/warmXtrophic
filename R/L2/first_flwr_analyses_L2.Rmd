...
mod1 <- lmer(log(julian_min) ~ state*year_factor + insecticide*year_factor + (1|plot), kbs_plot, REML = FALSE)
...
# effect size from emmeans(mod9p, list(pairwise ~ state), adjust = "tukey")
# calculating effect size - accounting for log transformation
exp((5.277390) +  (-0.0251)*0) # 195.858 
exp((5.277390) +  (-0.0251)*1) # 191.0032
# effect:
191.0032 - 195.858 # effect of warming - warmed plots increased the date of first flower by 4.8548 days
...
modtest1u <- lmer(log(julian_min) ~ mean_temp + (1|plot), kbs_plot, REML=FALSE)
modtest2u <- lmer(log(julian_min) ~ GDD_cumulative + (1|plot), kbs_plot, REML=FALSE)
...
mod_spp_k <- lmer(julian_min ~ state * insecticide + species + as.factor(year_factor) + (1|plot), flwr_kbs_spp, REML=FALSE)
anova(mod_spp_k)
# making a table
...
# UMBS
# species model for supp
mod_spp_u <- lmer(julian_min ~ state * insecticide + species + as.factor(year_factor) + (1|plot), flwr_umbs_spp, REML=FALSE)
anova(mod_spp_u)
# making a table

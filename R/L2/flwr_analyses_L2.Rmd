...
outlierTest(fit_spp_state_kbs) # outliers
...
anova(mod7b, mod7c) #not sure what these results are telling me, no pvalue... but going with model 7c
summary(mod7c)
anova(mod7c) # investigates whether at least one of the levels within each factor is significantly different.
...
plot_model(mod7c, sort.est = TRUE)
# if you want to standardize the estimates:
plot_model(mod7c, sort.est = TRUE, type="std") 
# these are the fixed predicted values: - note this is a new plot
plot_model(mod7c, type = "pred", terms = c("species", "state", "year_factor"))
# these are the random effects estimates
plot_model(mod7c, type = "re")
...
mod2p <- lmer(log(julian_median) ~ state + factor(year_factor) + (1|plot), umbs_flwr_plot, REML=FALSE)
mod3p <- lmer(log(julian_median) ~ state * factor(year_factor) + (1|plot), umbs_flwr_plot, REML=FALSE)
anova(mod1p, mod2p) #mod2p
anova(mod2p, mod3p) # pvalue > 0.05 so models do not differ, go with simpler model = mod2p (interaction does not make model better)
emmeans(mod2p, list(pairwise ~ state + factor(year_factor)), adjust = "tukey")

mod2p.2 <- lmer(log(julian_median) ~ state + factor(year_factor) + insecticide + (1|plot), umbs_flwr_plot, REML=FALSE)
...
anova(kmod7a, kmod7b) #kmod 7a but only slightly so (pvalue = 0.058)

...
# changing zeros in data to be 1s - some species were only recorded on a single data collection event (aka date)
# which comes out to be 0 days of flwr duration. Having zeros in the data makes it harder to work with so changing
# the 0s to one is more reflective of the flwr duration
#flwr_species[flwr_species$flwr_duration == 0] <- 1
#flwr_plot[flwr_plot$flwr_duration == 0] <- 1
...
#umbs_flwr_plot <- subset(umbs_flwr_plot, year!="2021") # delete 2021 data from umbs dataframe (doesn't make sense to have bc we can't get min flower/sd, median flower, or flower duration from 2021 UMBS)
...
# Kara editing: maybe log transforming or sqrt transforming the data could help here since its right skewed
...
# Kara edits: likely because some of these can't deal w/ 0's in data, they work with the flwr_duration_scaled 
...
shapiro.test(kbs_flwr_plot$flwr_duration) # pvalue is < 0.05 so we reject the null hypothesis that the data is normal (aka not normally distributed) but it is really close = 0.04149 so going to go with normally distributed
...
# none of these work below and idk why
# Kara's edits: added the scaled column here to get these to work
# Gamma distribution 
...
```{r, warning=F, error=TRUE}
### KBS ###
hist(kbs_flwr_plot_origin$flwr_duration)
qqnorm(kbs_flwr_plot_origin$flwr_duration)
shapiro.test(kbs_flwr_plot_origin$flwr_duration) # pvalue is < 0.05 so we reject the null hypothesis that the data is normal (aka not normally distributed) 

# Visualizing median Julian date for umbs at the PLOT LEVEL
ggplot(kbs_flwr_plot_origin, aes(flwr_duration, fill = plot)) + 
        geom_histogram(binwidth = 0.5) + 
        facet_grid(year ~ site, margins = TRUE, scales = "free")

ggplot(kbs_flwr_plot_origin, aes(flwr_duration, fill = as.factor(plot))) + geom_histogram(binwidth = 0.5) + 
        facet_grid(state~year, margins = TRUE, scales = "free")

ggplot(kbs_flwr_plot_origin, aes(flwr_duration, fill = plot, color=plot)) +
        geom_density(alpha = 0.1)
...
# Exploring distributions for these data:
descdist(kbs_flwr_plot_origin$flwr_duration, discrete = FALSE)

## none of these work below and idk why
## Gamma distribution 
#fit.gamma <- fitdist(kbs_flwr_plot_origin$flwr_duration, "gamma")
#plot(fit.gamma)
#
## Weibull distribution
#fit.weibull  <- fitdist(kbs_flwr_plot_origin$flwr_duration, "weibull")
#plot(fit.weibull)
#
## Lognormal distribution 
#fit.ln <- fitdist(kbs_flwr_plot_origin$flwr_duration, "lnorm")
#plot(fit.ln)
#
#par(mfrow=c(2,2))
#plot.legend <- c("Gamma", "Weibull", "Log Normal")
...
```{r, warning=F, error=TRUE}
### KBS ###
hist(kbs_flwr_plot_growthhabit$flwr_duration)
qqnorm(kbs_flwr_plot_growthhabit$flwr_duration)
shapiro.test(kbs_flwr_plot_growthhabit$flwr_duration) # pvalue is < 0.05 so we reject the null hypothesis that the data is normal (aka not normally distributed) 

# Visualizing median Julian date for umbs at the PLOT LEVEL
ggplot(kbs_flwr_plot_growthhabit, aes(flwr_duration, fill = plot)) + 
        geom_histogram(binwidth = 0.5) + 
        facet_grid(year ~ site, margins = TRUE, scales = "free")

ggplot(kbs_flwr_plot_growthhabit, aes(flwr_duration, fill = as.factor(plot))) + geom_histogram(binwidth = 0.5) + 
        facet_grid(state~year, margins = TRUE, scales = "free")

ggplot(kbs_flwr_plot_growthhabit, aes(flwr_duration, fill = plot, color=plot)) +
        geom_density(alpha = 0.1)
...
# Exploring distributions for these data:
descdist(kbs_flwr_plot_growthhabit$flwr_duration, discrete = FALSE)
...
```{r, warning=F, error=TRUE}
### UMBS ###
hist(umbs_flwr_spp$flwr_duration)
qqnorm(umbs_flwr_spp$flwr_duration)
shapiro.test(umbs_flwr_spp$flwr_duration) # pvalue is < 0.05 so we reject the null hypothesis that the data is normal (aka not normally distributed)

# Visualizing median Julian date for umbs at the PLOT LEVEL
ggplot(umbs_flwr_spp, aes(flwr_duration, fill = plot)) + 
        geom_histogram(binwidth = 0.5) + 
        facet_grid(year ~ site, margins = TRUE, scales = "free")

ggplot(umbs_flwr_spp, aes(flwr_duration, fill = as.factor(plot))) + geom_histogram(binwidth = 0.5) + 
        facet_grid(state~year, margins = TRUE, scales = "free")

ggplot(umbs_flwr_spp, aes(flwr_duration, fill = plot, color=plot)) +
        geom_density(alpha = 0.1)
...
# Exploring distributions for these right-skewed data:
descdist(umbs_flwr_spp$flwr_duration, discrete = FALSE)

# none of these work below and idk why
# Gamma distribution 
#fit.gamma <- fitdist(umbs_flwr_spp$flwr_duration, "gamma")
#plot(fit.gamma)
#
## Weibull distribution
#fit.weibull  <- fitdist(umbs_flwr_spp$flwr_duration, "weibull")
#plot(fit.weibull)
#
## Lognormal distribution 
#fit.ln <- fitdist(umbs_flwr_spp$flwr_duration, "lnorm")
#plot(fit.ln)
#
#par(mfrow=c(2,2))
#plot.legend <- c("Gamma", "Weibull", "Log Normal")
...
```{r, warning=F, error=TRUE}
### UMBS ###
hist(umbs_flwr_plot$flwr_duration)
qqnorm(umbs_flwr_plot$flwr_duration)
shapiro.test(umbs_flwr_plot$flwr_duration) # pvalue is > 0.05 so we accept the null hypothesis that the data is normally distributed! WOO

# Visualizing median Julian date for umbs at the PLOT LEVEL
ggplot(umbs_flwr_plot, aes(flwr_duration, fill = plot)) + 
        geom_histogram(binwidth = 0.5) + 
        facet_grid(year ~ site, margins = TRUE, scales = "free")

ggplot(umbs_flwr_plot, aes(flwr_duration, fill = as.factor(plot))) + geom_histogram(binwidth = 0.5) + 
        facet_grid(state~year, margins = TRUE, scales = "free")

ggplot(umbs_flwr_plot, aes(flwr_duration, fill = plot, color=plot)) +
        geom_density(alpha = 0.1)
...
# Exploring distributions for these data:
descdist(umbs_flwr_plot$flwr_duration, discrete = FALSE)

# Gamma distribution 
fit.gamma <- fitdist(umbs_flwr_plot$flwr_duration, "gamma")
plot(fit.gamma)

# Weibull distribution
fit.weibull  <- fitdist(umbs_flwr_plot$flwr_duration, "weibull")
plot(fit.weibull)

# Lognormal distribution 
fit.ln <- fitdist(umbs_flwr_plot$flwr_duration, "lnorm")
plot(fit.ln)
...
# Goodness of fit comparisons across fits (can't include the sqrt normal bc it becomes diff response values)
gofstat(list(fit.gamma, fit.weibull, fit.ln), fitnames = c("Gamma", "Weibull", "Log Normal"))
# Weibull is the best fit according to this comparison
```

# UMBS PLOT LEVEL ORIGIN - Looking at DURATION of flowering
```{r, warning=F, error=TRUE}
### KBS ###
hist(umbs_flwr_plot_origin$flwr_duration)
qqnorm(umbs_flwr_plot_origin$flwr_duration)
shapiro.test(kbs_flwr_plot_origin$flwr_duration) # pvalue is < 0.05 so we reject the null hypothesis that the data is normal (aka not normally distributed) 

# Visualizing median Julian date for umbs at the PLOT LEVEL
ggplot(umbs_flwr_plot_origin, aes(flwr_duration, fill = plot)) + 
        geom_histogram(binwidth = 0.5) + 
        facet_grid(year ~ site, margins = TRUE, scales = "free")

ggplot(umbs_flwr_plot_origin, aes(flwr_duration, fill = as.factor(plot))) + geom_histogram(binwidth = 0.5) + 
        facet_grid(state~year, margins = TRUE, scales = "free")

ggplot(umbs_flwr_plot_origin, aes(flwr_duration, fill = plot, color=plot)) +
        geom_density(alpha = 0.1)
...
# Exploring distributions for these data:
descdist(umbs_flwr_plot_origin$flwr_duration, discrete = FALSE)
...
```{r, warning=F, error=TRUE}
### KBS ###
hist(umbs_flwr_plot_growthhabit$flwr_duration)
qqnorm(umbs_flwr_plot_growthhabit$flwr_duration)
shapiro.test(umbs_flwr_plot_growthhabit$flwr_duration) # pvalue is < 0.05 so we reject the null hypothesis that the data is normal (aka not normally distributed) 

# Visualizing median Julian date for umbs at the PLOT LEVEL
ggplot(umbs_flwr_plot_growthhabit, aes(flwr_duration, fill = plot)) + 
        geom_histogram(binwidth = 0.5) + 
        facet_grid(year ~ site, margins = TRUE, scales = "free")

ggplot(umbs_flwr_plot_growthhabit, aes(flwr_duration, fill = as.factor(plot))) + geom_histogram(binwidth = 0.5) + 
        facet_grid(state~year, margins = TRUE, scales = "free")

ggplot(umbs_flwr_plot_growthhabit, aes(flwr_duration, fill = plot, color=plot)) +
        geom_density(alpha = 0.1)
...
# Exploring distributions for these data:
descdist(umbs_flwr_plot_growthhabit$flwr_duration, discrete = FALSE)

## none of these work below and idk why
## Gamma distribution 
#fit.gamma <- fitdist(umbs_flwr_plot_growthhabit$flwr_duration, "gamma")
#plot(fit.gamma)
#
## Weibull distribution
#fit.weibull  <- fitdist(umbs_flwr_plot_growthhabit$flwr_duration, "weibull")
#plot(fit.weibull)
#
## Lognormal distribution 
#fit.ln <- fitdist(umbs_flwr_plot_growthhabit$flwr_duration, "lnorm")
#plot(fit.ln)
#
#par(mfrow=c(2,2))
#plot.legend <- c("Gamma", "Weibull", "Log Normal")
...
# Plot level data
# KBS State-only model
fit_plot_state_kbs <- lm(sqrt(flwr_duration) ~ state, data = kbs_flwr_plot)
...
# KBS State and year model
fit_plot_stateyear_kbs <- lm(sqrt(flwr_duration) ~ state + year_factor, data = kbs_flwr_plot)
...
fit3 <- lm(flwr_duration ~ state * year_factor, data = kbs_flwr_plot)
...
# KBS State-only model
fit_plot_state_kbs_origin <- lm(flwr_duration ~ state, data = kbs_flwr_plot_origin)
...
# KBS State and year model
fit_plot_stateyear_kbs_origin <- lm(flwr_duration ~ state + year_factor, data = kbs_flwr_plot_origin)
...
fit3 <- lm(flwr_duration ~ state * year_factor, data = kbs_flwr_plot_origin)
interact_plot(fit3, pred = year_factor, modx = state) # this looks very strange to me
```
```{r}
# Plot level GROWTH HABIT data
# KBS State-only model
fit_plot_state_kbs_gh <- lm(flwr_duration ~ state, data = kbs_flwr_plot_growthhabit)
...
# KBS State and year model
fit_plot_stateyear_kbs_gh <- lm(flwr_duration ~ state + year_factor, data = kbs_flwr_plot_growthhabit)
...
fit3 <- lm(flwr_duration ~ state * year_factor, data = kbs_flwr_plot_growthhabit)
interact_plot(fit3, pred = year_factor, modx = state) # this looks very strange to me
```
...
fit_spp_state_umbs <- lm(sqrt(flwr_duration) ~ state, data = umbs_flwr_spp)
...
fit_spp_stateyear_umbs <- lm(sqrt(flwr_duration) ~ state + year_factor, data = umbs_flwr_spp)
...
fit3 <- lm(sqrt(flwr_duration) ~ state + year_factor + species, data = umbs_flwr_spp)
interact_plot(fit3, pred = year_factor, modx = state)
fit4 <- lm(sqrt(flwr_duration) ~ state * year_factor + species, data = umbs_flwr_spp)
interact_plot(fit4, pred = year_factor, modx = state, mod2 = species)
```
...
# Plot level data
# UMBS State-only model
fit_plot_state_umbs <- lm(flwr_duration ~ state, data = umbs_flwr_plot)
...
# UMBS State and year model
fit_plot_stateyear_umbs <- lm(flwr_duration ~ state + year_factor, data = umbs_flwr_plot)
...
fit3 <- lm(flwr_duration ~ state * year_factor, data = umbs_flwr_plot)
interact_plot(fit3, pred = year_factor, modx = state)
```
```{r}
# Plot level ORIGIN data
# UMBS State-only model
fit_plot_state_umbs_origin <- lm(flwr_duration ~ state, data = umbs_flwr_plot_origin)
...
# UMBS State and year model
fit_plot_stateyear_umbs_origin <- lm(flwr_duration ~ state + year_factor, data = umbs_flwr_plot_origin)
...
fit3 <- lm(flwr_duration ~ state * year_factor, data = umbs_flwr_plot_origin)
interact_plot(fit3, pred = year_factor, modx = state)
```
...
# Plot level GROWTH HABIT data
# UMBS State-only model
fit_plot_state_umbs_gh <- lm(flwr_duration ~ state, data = umbs_flwr_plot_growthhabit)
...
# UMBS State and year model
fit_plot_stateyear_umbs_gh <- lm(flwr_duration ~ state + year_factor, data = umbs_flwr_plot_growthhabit)
...
fit3 <- lm(flwr_duration ~ state * year_factor, data = umbs_flwr_plot_growthhabit)
interact_plot(fit3, pred = year_factor, modx = state)
```
...
# AICctab(mod1, mod2, mod3, mod5, mod7, mod7a, mod7b, mod7c, mod8, mod9, mod9a, mod10, mod11, mod11a, mod13, weights=T) # mod7a is the best fitting model (took out mod12 bc it wouldn't run)
# not sure why the above code wont workd - all the data looks identical to me despite what error says??
summary(mod7a)
anova(mod7a)
```
...
```{r, error=TRUE}
mod1p <- lmer(flwr_duration ~ state + (1|plot), kbs_flwr_plot, REML=FALSE)
mod2p <- lmer(flwr_duration ~ insecticide + (1|plot), kbs_flwr_plot, REML=FALSE)
mod3p <- lmer(flwr_duration ~ insecticide + state + (1|plot), kbs_flwr_plot, REML=FALSE)
mod4p <- lmer(flwr_duration ~ insecticide * state + (1|plot), kbs_flwr_plot, REML=FALSE)
mod5p <- lmer(flwr_duration ~ state + year_factor + (1|plot), kbs_flwr_plot, REML=FALSE)
mod6p <- lmer(flwr_duration ~ state + year_factor + insecticide + (1|plot), kbs_flwr_plot, REML=FALSE)
mod7p <- lmer(flwr_duration ~ state * year_factor + (1|plot), kbs_flwr_plot, REML=FALSE)
mod8p <- lmer(flwr_duration ~ state * year_factor + insecticide + (1|plot), kbs_flwr_plot, REML=FALSE)
mod9p <- lmer(flwr_duration ~ state * insecticide + year_factor + (1|plot), kbs_flwr_plot, REML=FALSE)
mod10p <- lmer(flwr_duration ~ state + insecticide * year_factor + (1|plot), kbs_flwr_plot, REML=FALSE)
mod11p <- lmer(flwr_duration ~ state * year_factor * insecticide + (1|plot), kbs_flwr_plot, REML=FALSE)
...
```{r}
# UMBS SPECIES LEVEL - Looking at DURATION of flowering
mod1u <- lmer(sqrt(flwr_duration) ~ state*year_factor + insecticide*year_factor + (1|species) + (1|plot), umbs_flwr_spp, REML = FALSE)
ggplot(umbs_flwr_spp,aes(x=plot,y=flwr_duration,col=species)) + geom_jitter() + geom_boxplot(alpha=0.2) + facet_wrap(~plot)
...
outlierTest(mod1u) # outliers row 1259, value = 0 
...
# Do we need to include plot as a random effect with the KBS models?   
mod1u <- lmer(sqrt(flwr_duration) ~ state*year_factor + insecticide*year_factor + (1|species) + (1|plot), umbs_flwr_spp, REML = FALSE)
mod2u <- lmer(sqrt(flwr_duration) ~ state*year_factor + insecticide*year_factor + (1|species), umbs_flwr_spp, REML=FALSE)
# Run analysis of variance on each model (see this for more explanation on how anova on a linear mixed effects model is similar to an anove on a regular linear model: https://m-clark.github.io/docs/mixedModels/anovamixed.html)
anova(mod1u)
anova(mod2u)
...
# Do we need to include insecticide? (dropping insecticide from the model)
# mod2 <- lmer(log(relabun) ~ state*year_factor + insecticide*year_factor + (1|species), comp_kbs_spp, REML=FALSE)
mod3u <- lmer(sqrt(flwr_duration) ~ state*year_factor + (1|species), umbs_flwr_spp, REML=FALSE)
anova(mod2u, mod3u)

AICctab(mod2u, mod3u, weights=T)
# Looks like no, pvalue > 0.05, so insecticide does not improve model fit so we will go with mod 3
...
# Does year need to be interactive with state?
mod5u <- lmer(sqrt(flwr_duration) ~ state + year_factor + (1|species), umbs_flwr_spp, REML=FALSE)
anova(mod3u, mod5u)
# No, P>0.05 so state*year_factor doesn't strongly improve model fit so we will shift to mod5, but AIC values are super close!!
...
# If we wanted to include plots nested within year it would look like this:
mod6u <- lmer(sqrt(flwr_duration) ~ state + year_factor + (1|species) + (1 + year_factor|plot), umbs_flwr_spp, REML=FALSE)
...
mod7u <- lmer(sqrt(flwr_duration) ~ state + species + (1+factor(year_factor)|plot), umbs_flwr_spp, REML=FALSE)
mod7au <- lmer(sqrt(flwr_duration)  ~ state + species + factor(year_factor) + (1|plot), umbs_flwr_spp, REML=FALSE)
mod7bu <- lmer(sqrt(flwr_duration)  ~ state * factor(year_factor) + species + (1|plot), umbs_flwr_spp, REML=FALSE)
mod7cu <- lmer(sqrt(flwr_duration)  ~ state + species + factor(year_factor) + insecticide + (1|plot), umbs_flwr_spp, REML=FALSE)
...
# Yes, at least one of the species is different (they do not all have the same relative abundances). 
emmeans(mod7cu, list(pairwise ~ state + year_factor), adjust = "tukey")
emmeans(mod7cu, list(pairwise ~ year_factor), adjust = "tukey")
emmeans(mod7cu, list(pairwise ~ species), adjust = "tukey")
emmeans(mod7cu, list(pairwise ~ insecticide), adjust = "tukey")
...
# including native vs. exotic
umbs_flwr_spp <- within(kbs_flwr_spp, origin <- relevel(factor(origin), ref = "Native")) # releveling so native is the reference
mod8u <- lmer(sqrt(flwr_duration) ~ state * origin + (1+factor(year_factor)|plot), umbs_flwr_spp, REML=FALSE)
mod9u <- lmer(sqrt(flwr_duration) ~ state + origin + (1+factor(year_factor)|plot), umbs_flwr_spp, REML=FALSE)
mod9au <- lmer(sqrt(flwr_duration) ~ state + origin + factor(year_factor) + (1|plot), umbs_flwr_spp, REML=FALSE)
anova(mod8u, mod9u) # model 9 is a better fit to data
anova(mod9u, mod9au) # mod 9a
summary(mod9au)
anova(mod9au)
emmeans(mod9au, list(pairwise ~ state + origin + year_factor), adjust = "tukey")

# including growth form - first with interaction term
umbs_flwr_spp <- within(umbs_flwr_spp, growth_habit <- relevel(factor(growth_habit), ref = "Forb")) # releveling so forb is the reference
mod10u <- lmer(sqrt(flwr_duration) ~ state * growth_habit + (1+factor(year_factor)|plot), umbs_flwr_spp, REML=FALSE)
mod11u <- lmer(sqrt(flwr_duration) ~ state + growth_habit + (1+factor(year_factor)|plot), umbs_flwr_spp, REML=FALSE)
mod11au <- lmer(sqrt(flwr_duration) ~ state + growth_habit + factor(year_factor) + (1|plot), umbs_flwr_spp, REML=FALSE)
anova(mod10u, mod11u) # model 11 is a better fit to data
anova(mod11u, mod11au) # model 11a is still a better fit to data
summary(mod11au)
anova(mod11au)
emmeans(mod11au, list(pairwise ~ factor(year_factor) * growth_habit), adjust = "tukey")

# New version of our model incorporating interaction term and species within year so that there is a separate intercept and slope for each species. The issue here is that there are some species that are not found each year. Easiest to remove those from another version of this dataframe before running below. Otherwise, it's not a balanced design.
# updated mod4
mod12u <- lmer(sqrt(flwr_duration) ~ state * factor(year_factor) + (1+factor(year_factor)|species), umbs_flwr_spp)

# So another version of this model would include the interaction but not include the nesting (and thus would assume that species aren't observed ea yr)
# updated mod5
mod13u <- lmer(sqrt(flwr_duration) ~ state * factor(year_factor) + (1|species), umbs_flwr_spp)

# All the models ran:
mod1u <- lmer(sqrt(flwr_duration) ~ state*year_factor + insecticide*year_factor + (1|species) + (1|plot), umbs_flwr_spp, REML=FALSE)
mod2u <- lmer(sqrt(flwr_duration) ~ state*year_factor + insecticide*year_factor + (1|species), umbs_flwr_spp, REML=FALSE)
mod3u <- lmer(sqrt(flwr_duration) ~ state*year_factor + (1|species), umbs_flwr_spp, REML=FALSE)
mod4u <- lmer(sqrt(flwr_duration) ~ state*year_factor + insecticide + (1|species) + (1|plot), umbs_flwr_spp, REML=FALSE)
mod5u <- lmer(sqrt(flwr_duration) ~ state + year_factor + (1|species), umbs_flwr_spp, REML=FALSE)
mod6u <- lmer(sqrt(flwr_duration) ~ state + year_factor + (1|species) + (1 + year|plot), umbs_flwr_spp, REML=FALSE)
mod7u <- lmer(sqrt(flwr_duration) ~ state + species + (1+factor(year_factor)|plot), umbs_flwr_spp, REML=FALSE)
mod7au <- lmer(sqrt(flwr_duration) ~ state + species + factor(year_factor) + (1|plot), umbs_flwr_spp, REML=FALSE)
mod7bu <- lmer(sqrt(flwr_duration) ~ state * factor(year_factor) + species + (1|plot), umbs_flwr_spp, REML=FALSE)
mod7cu <- lmer(sqrt(flwr_duration) ~ state + species + factor(year_factor) + insecticide + (1|plot), umbs_flwr_spp, REML=FALSE)
...
mod11u <- lmer(sqrt(flwr_duration) ~ state + growth_habit + (1+factor(year_factor)|plot), umbs_flwr_spp, REML=FALSE)
mod11au <- lmer(sqrt(flwr_duration) ~ state + growth_habit + factor(year_factor) + (1|plot), umbs_flwr_spp, REML=FALSE)
mod12u <- lmer(sqrt(flwr_duration) ~ state * factor(year_factor) + (1+factor(year_factor)|species), umbs_flwr_spp, REML=FALSE)
mod13u <- lmer(sqrt(flwr_duration) ~ state * factor(year_factor) + (1|species), umbs_flwr_spp, REML=FALSE)
...
```{r}
mod1up <- lmer(flwr_duration ~ state + (1|plot), umbs_flwr_plot, REML=FALSE)
mod2up <- lmer(flwr_duration ~ insecticide + (1|plot), umbs_flwr_plot, REML=FALSE)
mod3up <- lmer(flwr_duration ~ insecticide + state + (1|plot), umbs_flwr_plot, REML=FALSE)
mod4up <- lmer(flwr_duration ~ insecticide * state + (1|plot), umbs_flwr_plot, REML=FALSE)
mod5up <- lmer(flwr_duration ~ state + year_factor + (1|plot), umbs_flwr_plot, REML=FALSE)
mod6up <- lmer(flwr_duration ~ state + year_factor + insecticide + (1|plot), umbs_flwr_plot, REML=FALSE)
mod7up <- lmer(flwr_duration ~ state * year_factor + (1|plot), umbs_flwr_plot, REML=FALSE)
mod8up <- lmer(flwr_duration ~ state * year_factor + insecticide + (1|plot), umbs_flwr_plot, REML=FALSE)
mod9up <- lmer(flwr_duration ~ state * insecticide + year_factor + (1|plot), umbs_flwr_plot, REML=FALSE)
mod10up <- lmer(flwr_duration ~ state + insecticide * year_factor + (1|plot), umbs_flwr_plot, REML=FALSE)
mod11up <- lmer(flwr_duration ~ state * year_factor * insecticide + (1|plot), umbs_flwr_plot, REML=FALSE)
AICctab(mod1up, mod2up, mod3up, mod4up, mod5up, mod6up, mod7up, mod8up, mod9up, mod10up, mod11up, weights=T) # model 5p and 7p are the top models
...
# need to work on the code below *left off on 12/17/21
...
hist(umbs_flwr_plot_origin$flwr_duration)
qqnorm(umbs_flwr_plot_origin$flwr_duration)
shapiro.test(umbs_flwr_plot_origin$flwr_duration) # pvalue is < 0.05 so we reject the null hypothesis that the data is normally distributed (aka not normal)

# Exploring distributions for these data:
descdist(umbs_flwr_plot_origin$flwr_duration, discrete = FALSE)

# UMBS SPECIES LEVEL - Looking at DURATION of flowering
mod1u_or <- lmer(sqrt(flwr_duration) ~ state*year_factor + insecticide*year_factor + (1|plot), umbs_flwr_plot_origin, REML = FALSE)

# Check Assumptions:
# (1) Linearity: if covariates are not categorical (year isn't)
# (2) Homogeneity: Need to Check by plotting residuals vs predicted values.
par(mfrow=c(1,2))
plot(mod1u_or) # the zeros in the data are making this look weird
# Homogeneity of variance is ok here (increasing variance in resids is not increasing with fitted values)
# Check for homogeneity of variances (true if p>0.05). If the result is not significant, the assumption of equal variances (homoscedasticity) is met (no significant difference between the group variances).
# *****Levene’s Test - tests whether or not the variance among two or more groups is equal - If the p-value is less than our chosen significance level, we can reject the null hypothesis and conclude that we have enough evidence to state that the variance among the groups is not equal (which we want).

# leveneTest(residuals(mod1u_or) ~ umbs_flwr_spp$state)
# # Assumption met
# leveneTest(residuals(mod1u_or) ~ umbs_flwr_spp$species)
# # Assumption met
# leveneTest(residuals(mod1u_or) ~ umbs_flwr_spp$insecticide) 
# # Assumption not met - this seems weird
# leveneTest(residuals(mod1u_or) ~ umbs_flwr_spp$plot)
# # Assumption not met
# 
# # (3) Normality of error term: need to check by histogram, QQplot of residuals, could do Kolmogorov-Smirnov test.
# # Check for normal residuals
# qqPlot(resid(mod1u_or))
# hist(residuals(mod1u_or))
# shapiro.test(resid(mod1u_or)) # not normally distributed resids bc p<0.05
# outlierTest(mod1u_or) # outliers row 1259, value = 0 
# 
# # (4) Normality of random effect: Get the estimate of random effect (e.g., random intercepts), and check them as you would # check the residual. 
# require(lme4)
# r_int <- ranef(mod1u)$plot$`(Intercept)`
...
umod1p_origin <- lmer(flwr_duration ~ state * origin + (1+factor(year_factor)|plot), umbs_flwr_plot_origin, REML=FALSE)
umod2p_origin <- lmer(flwr_duration ~ state + origin + (1+factor(year_factor)|plot), umbs_flwr_plot_origin, REML=FALSE)
umod3p_origin <- lmer(flwr_duration ~ state + origin + factor(year_factor) + (1|plot), umbs_flwr_plot_origin, REML=FALSE)
...
# Growth habit
mod10u <- lmer(flwr_duration ~ state * growth_habit + (1+factor(year_factor)|plot), umbs_flwr_spp, REML=FALSE)
mod11u <- lmer(flwr_duration ~ state + growth_habit + (1+factor(year_factor)|plot), umbs_flwr_spp, REML=FALSE)
mod11au <- lmer(flwr_duration ~ state + growth_habit + factor(year_factor) + (1|plot), umbs_flwr_spp, REML=FALSE)
```

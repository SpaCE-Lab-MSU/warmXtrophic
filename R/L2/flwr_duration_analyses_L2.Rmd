...
shapiro.test(kbs_flwr_spp$flwr_duration_scaled) # pvalue is < 0.05 so we reject the null hypothesis that the data is normal (aka not normally distributed)
...
# weibull best fit, probably going to go with log transformation
```
...
outlierTest(fit_plot_state_kbs) # outliers - 142, 155, 220, 116, 207, 272
...
AICctab(mod9a, mod11a, weights=T)
anova(mod9a, mod11a)
summary(mod7a)
anova(mod7a)
```
...
```{r, error=TRUE}
mod1p <- lmer(log(flwr_duration_scaled) ~ state + (1|plot), kbs_flwr_plot, REML=FALSE)
mod2p <- lmer(log(flwr_duration_scaled) ~ insecticide + (1|plot), kbs_flwr_plot, REML=FALSE)
mod3p <- lmer(log(flwr_duration_scaled) ~ insecticide + state + (1|plot), kbs_flwr_plot, REML=FALSE)
mod4p <- lmer(log(flwr_duration_scaled) ~ insecticide * state + (1|plot), kbs_flwr_plot, REML=FALSE)
mod5p <- lmer(log(flwr_duration_scaled) ~ state + year_factor + (1|plot), kbs_flwr_plot, REML=FALSE)
mod6p <- lmer(log(flwr_duration_scaled) ~ state + year_factor + insecticide + (1|plot), kbs_flwr_plot, REML=FALSE)
mod7p <- lmer(log(flwr_duration_scaled) ~ state * year_factor + (1|plot), kbs_flwr_plot, REML=FALSE)
mod8p <- lmer(log(flwr_duration_scaled) ~ state * year_factor + insecticide + (1|plot), kbs_flwr_plot, REML=FALSE)
mod9p <- lmer(log(flwr_duration_scaled) ~ state * insecticide + year_factor + (1|plot), kbs_flwr_plot, REML=FALSE)
mod10p <- lmer(log(flwr_duration_scaled) ~ state + insecticide * year_factor + (1|plot), kbs_flwr_plot, REML=FALSE)
mod11p <- lmer(log(flwr_duration_scaled) ~ state * year_factor * insecticide + (1|plot), kbs_flwr_plot, REML=FALSE)
AICctab(mod1p, mod2p, mod3p, mod4p, mod5p, mod6p, mod7p, mod8p, mod9p, mod10p, mod11p, weights=T) # model 5p and 7p are the top models
anova(mod5p, mod7p) # go with 5p although they are nearly identical
...
summary(mod12u)
anova(mod12u)
emmeans(mod12u, list(pairwise ~ state * factor(year_factor)), adjust = "tukey")
emmeans(mod12u, list(pairwise ~ state ), adjust = "tukey")
```

# UMBS Plot-level Mixed Effects Models:
```{r}
mod1up <- lmer(log(flwr_duration_scaled) ~ state + (1|plot), umbs_flwr_plot, REML=FALSE)
mod2up <- lmer(log(flwr_duration_scaled) ~ insecticide + (1|plot), umbs_flwr_plot, REML=FALSE)
mod3up <- lmer(log(flwr_duration_scaled) ~ insecticide + state + (1|plot), umbs_flwr_plot, REML=FALSE)
mod4up <- lmer(log(flwr_duration_scaled) ~ insecticide * state + (1|plot), umbs_flwr_plot, REML=FALSE)
mod5up <- lmer(log(flwr_duration_scaled) ~ state + year_factor + (1|plot), umbs_flwr_plot, REML=FALSE)
mod6up <- lmer(log(flwr_duration_scaled) ~ state + year_factor + insecticide + (1|plot), umbs_flwr_plot, REML=FALSE)
mod7up <- lmer(log(flwr_duration_scaled) ~ state * year_factor + (1|plot), umbs_flwr_plot, REML=FALSE)
mod8up <- lmer(log(flwr_duration_scaled) ~ state * year_factor + insecticide + (1|plot), umbs_flwr_plot, REML=FALSE)
mod9up <- lmer(log(flwr_duration_scaled) ~ state * insecticide + year_factor + (1|plot), umbs_flwr_plot, REML=FALSE)
mod10up <- lmer(log(flwr_duration_scaled) ~ state + insecticide * year_factor + (1|plot), umbs_flwr_plot, REML=FALSE)
mod11up <- lmer(log(flwr_duration_scaled) ~ state * year_factor * insecticide + (1|plot), umbs_flwr_plot, REML=FALSE)
AICctab(mod1up, mod2up, mod3up, mod4up, mod5up, mod6up, mod7up, mod8up, mod9up, mod10up, mod11up, weights=T)
anova(mod5up, mod6up) # go with simpler model 5up
AICctab(mod5up, mod6up, weights=T)
summ(mod5up)
summary(mod5up)
emmeans(mod5up, list(pairwise ~ state), adjust = "tukey")

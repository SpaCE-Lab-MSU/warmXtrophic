...
# Plot level data
# KBS State-only model
fit_plot_state_kbs <- lm(flwr_duration ~ state, data = kbs_flwr_plot)
...
# KBS State and year model
fit_plot_stateyear_kbs <- lm(flwr_duration ~ state + year_factor, data = kbs_flwr_plot)
...
AICctab(mod1p, mod2p, mod3p, mod4p, mod8p, weights=T) # model 5p and 7p are the top models
...
mod8u <- lmer(flwr_duration ~ state * origin + (1+factor(year_factor)|plot), umbs_flwr_plot_origin, REML=FALSE)
mod9u <- lmer(flwr_duration ~ state + origin + (1+factor(year_factor)|plot), umbs_flwr_plot_origin, REML=FALSE)
mod9au <- lmer(flwr_duration ~ state + origin + factor(year_factor) + (1|plot), umbs_flwr_plot_origin, REML=FALSE)

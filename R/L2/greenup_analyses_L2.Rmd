...
# state*year doesn't improve model fit so we could drop it and go with mod5, but note that the AIC values are super close
summary(mod5)
anova(mod3)
...
anova(mod3, mod6)
anova(mod6)
# mod 3 still better fit
plot_model(mod6, sort.est = TRUE)
# these are the fixed predicted values:
plot_model(mod6, type = "pred", terms = c("year_factor", "state"))
# these are the random effects estimates
plot_model(mod6, type = "re", terms = c("species", "plot"))

# mod3 (and mod6) are pretty complex in terms of interpretation (they actually don't have many parameters though). We could consider an alternative model that's simpler to understand and also one that provides more insight about the species.
...
# Start KD models - listed out possible models all at once, then compared:

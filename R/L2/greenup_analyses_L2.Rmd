...
# KD: not sure what this code below was for 
# code below won't run:
# Or try with tidyverse format
#green_kbsp.t<-as_tibble(green_kbsp)
#green_kbsp.t %>%
#    gather(state, plot_half_cover_date, year_factor) %>%
#    ggplot(aes(plot_half_cover_date, fill = state)) + geom_histogram() + facet_wrap(~year_factor)
...
ols_test_normality(fit1umbs) # p < 0.05 for all, so data is normal (I think)
...
leveragePlots(fit2)
ols_test_normality(fit2) # p < 0.05 for all, so data is normal (I think)
...
fitp2<- lm(plot_half_cover_date~state+year_factor, data = green_kbsp)
outlierTest(fitp2) # no outliers
qqPlot(fitp2, main="QQ Plot") 
hist(fitp2$residuals)
leveragePlots(fitp2)
leveneTest(residuals(fitp2) ~ green_kbsp$state)
leveneTest(residuals(fitp2) ~ green_kbsp$year_factor) # not met
ols_test_normality(fitp2)
shapiro.test(resid(fitp2))

# UMBS State and year model
fitp2u<- lm(plot_half_cover_date~state+year, data = green_umbsp)
outlierTest(fitp2u)
qqPlot(fitp2u, main="QQ Plot") 
hist(fitp2u$residuals)
leveragePlots(fitp2u)
ols_test_normality(fitp2u)
...
# UMBS
fit3u <- lm(plot_half_cover_date ~ state + year, data = green_umbsp)
interact_plot(fit3u, pred = year, modx = state)
fit4u <- lm(plot_half_cover_date ~ state * year, data = green_umbsp)
...
modfull <- lmer(plot_half_cover_date ~ state*year_factor + insecticide*year_factor + (1|plot), green_kbsp, REML=F)
mod1p <- lmer(plot_half_cover_date ~ state + (1|plot), green_kbsp, REML=FALSE)
mod2p <- lmer(plot_half_cover_date ~ insecticide + (1|plot), green_kbsp, REML=FALSE)
mod3p <- lmer(plot_half_cover_date ~ insecticide + state + (1|plot), green_kbsp, REML=FALSE)
mod4p <- lmer(plot_half_cover_date ~ insecticide * state + (1|plot), green_kbsp, REML=FALSE)
mod5p <- lmer(plot_half_cover_date ~ state + year_factor + (1|plot), green_kbsp, REML=FALSE)
mod6p <- lmer(plot_half_cover_date ~ state + year_factor + insecticide + (1|plot), green_kbsp, REML=FALSE)
mod7p <- lmer(plot_half_cover_date ~ state * year_factor + (1|plot), green_kbsp, REML=FALSE)
mod8p <- lmer(plot_half_cover_date ~ state * year_factor + insecticide + (1|plot), green_kbsp, REML=FALSE)
mod9p <- lmer(plot_half_cover_date ~ state * insecticide + year_factor + (1|plot), green_kbsp, REML=FALSE)
mod10p <- lmer(plot_half_cover_date ~ state + insecticide * year_factor + (1|plot), green_kbsp, REML=FALSE)
mod11p <- lmer(plot_half_cover_date ~ state * year_factor * insecticide + (1|plot), green_kbsp, REML=FALSE)
AICtab(modfull, mod1p, mod2p, mod3p, mod4p, mod5p, mod6p, mod7p, mod8p, mod9p, mod10p, mod11p, weights=T) # 5p, 6p, or 9p
anova(mod5p, mod6p) #5p - used this model in results section
anova(mod9p, mod5p) # should I go with 9p? includes insecticide as well
AICtab(mod5p, mod6p, mod9p)

summary(mod9p)
anova(mod9p)
anova(mod5p)
emmeans(mod9p, list(pairwise ~ state*insecticide), adjust = "tukey")
mod9p.emm <- emmeans(mod9p, ~ state * insecticide) # why are the comparisons the same for each year?
...
```{r}
mod1pu <- lmer(plot_half_cover_date ~ state + (1|plot), green_umbsp, REML=FALSE)
mod2pu <- lmer(plot_half_cover_date ~ state + factor(year_factor) + (1|plot), green_umbsp, REML=FALSE)
mod3pu <- lmer(plot_half_cover_date ~ state * year_factor + (1|plot), green_umbsp, REML=FALSE)
anova(mod1pu, mod2pu, mod3pu) #mod2pu
summary(mod2pu)
```

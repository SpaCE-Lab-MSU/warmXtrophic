...
greenup <- read.csv(file.path(L2_dir, "greenup/final_greenup_species_L2.csv"))
greenup <- greenup %>% select(-X) # get rid of "X" column that shows up
greenupp <- read.csv(file.path(L2_dir, "greenup/final_greenup_plot_L2.csv"))
greenupp <- greenupp %>% select(-X) # get rid of "X" column that shows up
...
p1 + facet_wrap(~year_factor) + labs(title="Species-level half cover date")
...
p3 + facet_wrap(~year_factor) + labs(title="Species-level half cover date")
...
outlierTest(fit1a) # record 473 is an outlier, could probably ignore since we aren't using min date as a response variable
...
# these are the fixed effects estimates from summary(mod4)
...
mod6 <- lmer(spp_half_cover_date ~ state*year + insecticide + (1|species) + (1+year|plot), green_kbs, REML=FALSE)
anova(mod4, mod6)
anova(mod6)
...
plot_model(mod6, sort.est = TRUE)
# these are the fixed predicted values:
plot_model(mod6, type = "pred", terms = c("year", "state", "insecticide"))
# these are the random effects estimates
plot_model(mod6, type = "re", terms = c("species", "plot"))

# mod4 (and mod6) are pretty complex in terms of interpretation (they actually don't have many parameters though). We could consider an alternative model that's simpler to understand and also one that provides more insight about the species. That would be something like this:
mod7 <- lmer(spp_half_cover_date ~ state + species + (1+year|plot), green_kbs, REML=FALSE)
mod7a <- lmer(spp_half_cover_date ~ state + species + as.factor(year) + (1|plot), green_kbs, REML=FALSE)
mod7b <- lmer(spp_half_cover_date ~ state * as.factor(year) + species + (1|plot), green_kbs, REML=FALSE)
mod7c <- lmer(spp_half_cover_date ~ state + species + as.factor(year) + insecticide + (1|plot), green_kbs, REML=FALSE)
anova(mod6, mod7) # model 7 is a better fit to data
anova(mod7, mod7a) #mod 7a
anova(mod7a, mod7b) #mod 7a
anova(mod7a, mod7c) #mod 7a slightly better
summary(mod7a)
summary(mod7b)
summary(mod7c)
...
emmeans(mod7a, list(pairwise ~ as.factor(year)), adjust = "tukey")
...
plot_model(mod7a, type = "pred", terms = c("species", "state", "year"))
# these are the random effects estimates
...
mod8 <- lmer(spp_half_cover_date ~ state * origin + (1+year|plot), green_kbs, REML=FALSE)
summary(mod8)
anova(mod8)

# including native vs. exotic - first with interaction term
mod9 <- lmer(spp_half_cover_date ~ state + origin + (1+as.factor(year)|plot), green_kbs, REML=FALSE)
mod9a <- lmer(spp_half_cover_date ~ state + origin + as.factor(year) + (1|plot), green_kbs, REML=FALSE)
...
mod10 <- lmer(spp_half_cover_date ~ state * growth_habit + (1+as.factor(year)|plot), green_kbs, REML=FALSE)
summary(mod10)
anova(mod10)

# including native vs. exotic - first with interaction term
mod11 <- lmer(spp_half_cover_date ~ state + growth_habit + (1+as.factor(year)|plot), green_kbs, REML=FALSE)
mod11a <- lmer(spp_half_cover_date ~ state + growth_habit + as.factor(year) + (1|plot), green_kbs, REML=FALSE)
anova(mod10, mod11) # model 11 is a better fit to data
anova(mod11, mod11a)
summary(mod11a)
anova(mod11)
emmeans(mod11a, list(pairwise ~ state + growth_habit), adjust = "tukey")
...
umod7 <- lmer(spp_half_cover_date ~ state + species + (1+year|plot), green_umbs, REML=FALSE)
umod7a <- lmer(spp_half_cover_date ~ state + species + as.factor(year) + (1|plot), green_umbs, REML=FALSE)
umod7b <- lmer(spp_half_cover_date ~ state * as.factor(year) + species + (1|plot), green_umbs, REML=FALSE)
umod7c <- lmer(spp_half_cover_date ~ state + species + as.factor(year) + insecticide + (1|plot), green_umbs, REML=FALSE)
...
# Yes, at least one of the species is different (they do not all have the same half cover dates). 
emmeans(umod7a, list(pairwise ~ as.factor(year)), adjust = "tukey")
...
umod8 <- lmer(spp_half_cover_date ~ state * origin + (1+year|plot), green_umbs, REML=FALSE)
summary(umod8)
anova(umod8)

# including native vs. exotic - first with interaction term
umod9 <- lmer(spp_half_cover_date ~ state + origin + (1+as.factor(year)|plot), green_umbs, REML=FALSE)
umod9a <- lmer(spp_half_cover_date ~ state + origin + as.factor(year) + (1|plot), green_umbs, REML=FALSE)
...
umod10 <- lmer(spp_half_cover_date ~ state * growth_habit + (1+year|plot), green_umbs, REML=FALSE)
summary(umod10)
anova(umod10)

# including native vs. exotic - first with interaction term
umod11 <- lmer(spp_half_cover_date ~ state + growth_habit + (1+as.factor(year)|plot), green_umbs, REML=FALSE)
umod11a <- lmer(spp_half_cover_date ~ state + growth_habit + as.factor(year) + (1|plot), green_umbs, REML=FALSE)

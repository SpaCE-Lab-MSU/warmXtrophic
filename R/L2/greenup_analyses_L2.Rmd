...
outlierTest(fitp2) # one outlier - 85
...
# note: the greenup vs data is normal and doesn't need transformed
...
# using model 7a for overall species - level greenup model #
...
# also, note all random effects are plot - I have diff models in plot-level section below
...
summary(mod_kd11) # same as model 7a - confirm this model as the best 
...
```{r}
# KD note: these models are commented out because we re-summarized the data into plot-level dataframes (below)
# plot-level models using the species summarized data frame with specific random effects
# note: we decided this was not the best approach - better to use the summarized dataframes
#mod1ps <- lmer(spp_half_cover_date ~ state + (1|plot/species), green_kbs, REML=FALSE)
#mod2ps <- lmer(spp_half_cover_date ~ state + insecticide + (1|plot/species), green_kbs, REML=FALSE)
#mod3ps <- lmer(spp_half_cover_date ~ state + insecticide + year_factor + (1|plot/species), green_kbs, #REML=FALSE)
#mod4ps <- lmer(spp_half_cover_date ~ state + year_factor + (1|plot/species), green_kbs, REML=FALSE)
#mod5ps <- lmer(spp_half_cover_date ~ state + year_factor + (1|species) + (1|plot), green_kbs, REML=FALSE)
#mod6ps <- lmer(spp_half_cover_date ~ state + year_factor + insecticide + (1|species) + (1|plot), #green_kbs, REML=FALSE)
#mod7ps <- lmer(spp_half_cover_date ~ state + year_factor + insecticide + (1|species), green_kbs, #REML=FALSE)
#
#anova(mod1ps,mod2ps) #1
#anova(mod1ps,mod3ps) #3
#anova(mod4ps,mod3ps) #4
#anova(mod4ps,mod5ps) #5
#anova(mod6ps,mod5ps) #5
#anova(mod5ps,mod7ps) # the same...? 
#AICctab(mod1ps, mod2ps, mod3ps, mod4ps, mod5ps, mod6ps, mod7ps,weights=T)
#summary(mod5ps)
#anova(mod4ps)
...
mod12p <- lmer(med_half_cover_date ~ state + GDD_cumulative + (1|plot), green_kbsp2, REML=FALSE)
mod13p <- lmer(med_half_cover_date ~ state + mean_temp + (1|plot), green_kbsp2, REML=FALSE)
mod14p <- lmer(med_half_cover_date ~ state * GDD_cumulative + (1|plot), green_kbsp2, REML=FALSE)
mod15p <- lmer(med_half_cover_date ~ state * mean_temp + (1|plot), green_kbsp2, REML=FALSE)
mod16p <- lmer(med_half_cover_date ~ state + GDD_cumulative + mean_temp + (1|plot), green_kbsp2, REML=FALSE)
mod17p <- lmer(med_half_cover_date ~ state + as.factor(year_factor) + GDD_cumulative + (1|plot), green_kbsp2, REML=FALSE)
mod18p <- lmer(med_half_cover_date ~ state + as.factor(year_factor) + mean_temp + (1|plot), green_kbsp2, REML=FALSE)
mod19p <- lmer(med_half_cover_date ~ state * as.factor(year_factor) + GDD_cumulative + (1|plot), green_kbsp2, REML=FALSE)
mod20p <- lmer(med_half_cover_date ~ state + as.factor(year_factor) * GDD_cumulative + (1|plot), green_kbsp2, REML=FALSE)
mod21p <- lmer(med_half_cover_date ~ state * as.factor(year_factor) + mean_temp + (1|plot), green_kbsp2, REML=FALSE)
mod22p <- lmer(med_half_cover_date ~ state + as.factor(year_factor) * mean_temp + (1|plot), green_kbsp2, REML=FALSE)
mod23p <- lmer(med_half_cover_date ~ state * as.factor(year_factor) + insecticide + mean_temp + (1|plot), green_kbsp2, REML=FALSE)
mod24p <- lmer(med_half_cover_date ~ state * as.factor(year_factor) + insecticide + GDD_cumulative + (1|plot), green_kbsp2, REML=FALSE)
mod25p <- lmer(med_half_cover_date ~ state * mean_temp + insecticide + (1|plot), green_kbsp2, REML=FALSE)
mod26p <- lmer(med_half_cover_date ~ state * GDD_cumulative + insecticide + (1|plot), green_kbsp2, REML=FALSE)

# testing - does mean temp or state fit better in a model?
modtest1 <- lmer(med_half_cover_date ~ mean_temp * as.factor(year_factor) + insecticide + (1|plot), green_kbsp2, REML=FALSE)
modtest2 <- lmer(med_half_cover_date ~ state * as.factor(year_factor) + insecticide + (1|plot), green_kbsp2, REML=FALSE)
modtest3 <- lmer(med_half_cover_date ~ GDD_cumulative * as.factor(year_factor) + insecticide + (1|plot), green_kbsp2, REML=FALSE)
AICtab(modtest1,modtest2,modtest3)
anova(modtest1)
summary(modtest1)

AICtab(modfull, mod1p, mod2p, mod3p, mod4p, mod5p, mod6p, mod7p, mod8p, mod9p, mod10p, mod11p,
       mod12p,mod13p,mod14p,mod15p,mod16p,mod17p,mod18p,mod19p,mod20p,mod21p,mod22p,mod23p,mod24p,
       mod25p,mod26p,weights=T)
...
anova(mod8p, mod9p) #mod8p 
anova(mod8p, mod10p) #mod8p
anova(mod8p, mod11p) #mod8p
anova(mod8p, mod12p) #mod8p
anova(mod8p, mod13p) #mod8p
anova(mod8p, mod14p) #mod8p
anova(mod8p, mod15p) #mod8p
anova(mod8p, mod16p) #mod8p
anova(mod8p, mod17p) #mod8p
anova(mod8p, mod18p) #mod8p
anova(mod8p, mod19p) #mod8p
anova(mod8p, mod20p) #mod8p
anova(mod8p, mod21p) #8p
anova(mod8p, mod22p) #8p
anova(mod8p, mod23p) #either
anova(mod8p, mod24p) #either
anova(mod8p, mod25p) #8
anova(mod8p, mod26p) #8
table <- anova(mod9p, mod7p)
summary(mod8p)
anova(mod8p)
...
# comparisons
emmeans(mod21p, list(pairwise ~ state*insecticide), adjust = "tukey")
mod21p.emm <- emmeans(mod8p, ~ state*year_factor)
contrast(mod21p.emm, "consec", simple = "each", combine = F, adjust = "mvt")
emmip(mod9p, insecticide~state)
...
# adding in our temp data into some models
mod12p_u <- lmer(med_half_cover_date ~ state + GDD_cumulative + (1|plot), green_umbsp2, REML=FALSE)
mod13p_u <- lmer(med_half_cover_date ~ state + mean_temp + (1|plot), green_umbsp2, REML=FALSE)
mod14p_u <- lmer(med_half_cover_date ~ state + GDD_cumulative + mean_temp + (1|plot), green_umbsp2, REML=FALSE)
mod15p_u <- lmer(med_half_cover_date ~ state + as.factor(year_factor) + GDD_cumulative + (1|plot), green_umbsp2, REML=FALSE)
mod16p_u <- lmer(med_half_cover_date ~ state + as.factor(year_factor) + mean_temp + (1|plot), green_umbsp2, REML=FALSE)
mod17p_u <- lmer(med_half_cover_date ~ state * as.factor(year_factor) + GDD_cumulative + (1|plot), green_umbsp2, REML=FALSE)
mod18p_u <- lmer(med_half_cover_date ~ state + as.factor(year_factor) * GDD_cumulative + (1|plot), green_umbsp2, REML=FALSE)
mod19p_u <- lmer(med_half_cover_date ~ state * as.factor(year_factor) + mean_temp + (1|plot), green_umbsp2, REML=FALSE)
mod20p_u <- lmer(med_half_cover_date ~ state + as.factor(year_factor) * mean_temp + (1|plot), green_umbsp2, REML=FALSE)
mod21p_u <- lmer(med_half_cover_date ~ state * as.factor(year_factor) + mean_temp + insecticide +(1|plot), green_umbsp2, REML=FALSE)
mod22p_u <- lmer(med_half_cover_date ~ state * as.factor(year_factor) + GDD_cumulative + insecticide +(1|plot), green_umbsp2, REML=FALSE)
mod23p_u <- lmer(med_half_cover_date ~ state * mean_temp + insecticide +(1|plot), green_umbsp2, REML=FALSE)
mod24p_u <- lmer(med_half_cover_date ~ state * GDD_cumulative + insecticide +(1|plot), green_umbsp2, REML=FALSE)
mod25p_u <- lmer(med_half_cover_date ~ state * insecticide * as.factor(year_factor) + GDD_cumulative + (1|plot), green_umbsp2, REML=FALSE)
mod26p_u <- lmer(med_half_cover_date ~ state * insecticide * as.factor(year_factor) + mean_temp + (1|plot), green_umbsp2, REML=FALSE)


AICtab(modfull_u, mod1p_u, mod2p_u, mod3p_u, mod4p_u, mod5p_u, mod6p_u, mod7p_u, mod8p_u, mod9p_u, mod10p_u, mod11p_u,mod12p_u,mod13p_u,mod14p_u,mod15p_u,mod16p_u,mod17p_u,mod18p_u,mod19p_u,mod20p_u,mod21p_u,mod22p_u,mod23p_u,mod24p_u,mod25p_u,mod26p_u,weights=T)
...
anova(mod11p_u, mod12p_u) #mod11p_u
anova(mod11p_u, mod13p_u) #mod11p_u
anova(mod11p_u, mod14p_u) #mod11p_u
anova(mod11p_u, mod15p_u) #mod11p_u
anova(mod11p_u, mod16p_u) #mod11p_u
anova(mod11p_u, mod17p_u) #mod11p_u
anova(mod11p_u, mod18p_u) #mod11p_u
anova(mod11p_u, mod19p_u) #mod11p_u
anova(mod11p_u, mod20p_u) #mod11p_u
anova(mod11p_u, mod21p_u) #mod11p_u
anova(mod11p_u, mod22p_u) #11
anova(mod11p_u, mod23p_u) #11
anova(mod11p_u, mod24p_u) #11
anova(mod11p_u, mod25p_u) #11
anova(mod11p_u, mod26p_u) #11
table <- anova(mod9p_u, mod7p_u)
summary(mod26p_u)
anova(mod26p_u)

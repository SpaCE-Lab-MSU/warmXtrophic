...
greenup <- read.csv(file.path(L2_dir, "final_greenup_species_L2.csv"))
greenup <- greenup %>% select(-X) # get rid of "X" column that shows up
greenupp <- read.csv(file.path(L2_dir, "final_greenup_plot_L2.csv"))
...
green_kbsp<-subset(green_kbsp,select=c("plot_half_cover_date","min_green_date","plot","year","state","insecticide","site","year_factor"))
head(green_kbsp)

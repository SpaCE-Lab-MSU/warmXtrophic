...
mod7a <- lmer(spp_half_cover_date ~ state + species + year + (1|plot), green_kbs, REML=FALSE)
...
plot_model(mod7, sort.est = TRUE, type="std") 
# these are the fixed predicted values: - note this is a new plot
plot_model(mod7, type = "pred", terms = c("species", "state"))
# these are the random effects estimates
...
# including native vs. exotic - first with interaction term
mod9 <- lmer(spp_half_cover_date ~ state + origin + (1+year|plot), green_kbs, REML=FALSE)
mod9a <- lmer(spp_half_cover_date ~ state + origin + year + (1|plot), green_kbs, REML=FALSE)
...
mod10 <- lmer(spp_half_cover_date ~ state * growth_habit + (1+year|plot), green_kbs, REML=FALSE)
summary(mod10)
anova(mod10)

# including native vs. exotic - first with interaction term
mod11 <- lmer(spp_half_cover_date ~ state + growth_habit + (1+year|plot), green_kbs, REML=FALSE)
mod11a <- lmer(spp_half_cover_date ~ state + growth_habit + year + (1|plot), green_kbs, REML=FALSE)

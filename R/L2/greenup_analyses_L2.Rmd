...
###### green-up V2 models #####
# testing if year as continuous or year as a factor makes more sense (test models)
year.mod.test1 <- lmer(1/(med_half_cover_date) ~ state + year_factor+(1|plot), green_kbsp2, REML=FALSE)
year.mod.test2 <- lmer(1/(med_half_cover_date) ~ state + as.factor(year_factor)+(1|plot), green_kbsp2, REML=FALSE)
anova(year.mod.test1,year.mod.test2)
# going to use year as a factor
# I also think this makes the most sense because I don't think we expect there to be a much of a trend over time for green-up; maybe if we had a longer time span, but I don't think 6 years is enough to detect a trend over time and the results will be more influenced by between year variation in temp, precip, etc.
...
mod14p <- lmer(med_half_cover_date ~ state + GDD_cumulative + mean_temp + (1|plot), green_kbsp2, REML=FALSE)
mod15p <- lmer(med_half_cover_date ~ state + as.factor(year_factor) + GDD_cumulative + (1|plot), green_kbsp2, REML=FALSE)
mod16p <- lmer(med_half_cover_date ~ state + as.factor(year_factor) + mean_temp + (1|plot), green_kbsp2, REML=FALSE)
mod17p <- lmer(med_half_cover_date ~ state * as.factor(year_factor) + GDD_cumulative + (1|plot), green_kbsp2, REML=FALSE)
mod18p <- lmer(med_half_cover_date ~ state + as.factor(year_factor) * GDD_cumulative + (1|plot), green_kbsp2, REML=FALSE)
mod19p <- lmer(med_half_cover_date ~ state * as.factor(year_factor) + mean_temp + (1|plot), green_kbsp2, REML=FALSE)
mod20p <- lmer(med_half_cover_date ~ state + as.factor(year_factor) * mean_temp + (1|plot), green_kbsp2, REML=FALSE)
mod21p <- lmer(med_half_cover_date ~ state * as.factor(year_factor) + mean_temp + insecticide +(1|plot), green_kbsp2, REML=FALSE)
mod22p <- lmer(med_half_cover_date ~ state * as.factor(year_factor) + GDD_cumulative + insecticide +(1|plot), green_kbsp2, REML=FALSE)
mod23p <- lmer(med_half_cover_date ~ state * mean_temp + insecticide +(1|plot), green_kbsp2, REML=FALSE)
mod24p <- lmer(med_half_cover_date ~ state * GDD_cumulative + insecticide +(1|plot), green_kbsp2, REML=FALSE)


AICtab(modfull, mod1p, mod2p, mod3p, mod4p, mod5p, mod6p, mod7p, mod8p, mod9p, mod10p, mod11p,
       mod12p,mod13p,mod14p,mod15p,mod16p,mod17p,mod18p,mod19p,mod20p,mod21p,mod22p,mod23p,mod24p,weights=T)
...
anova(mod8p, mod21p) #either
anova(mod8p, mod22p) #either
anova(mod8p, mod23p) #mod8p
anova(mod8p, mod24p) #mod8p
anova(mod21p,mod21p)
table <- anova(mod9p, mod7p)
summary(mod21p)
anova(mod21p)

...
GDD <- read.csv(file.path(L1_dir, "HOBO_data/KBS_GDD_L1.csv"))
temps <- read.csv(file.path(L1_dir, "HOBO_data/KBS_greenup_temps_L1.csv"))
...
green_kbsp <- left_join(green_kbsp, temps, by=c('year','state'))
# basing cutoff for GDD at julian date 210
green_kbsp$GDD <- NA
green_kbsp$GDD[green_kbsp$year == 2016 & green_kbsp$state == "ambient"] = 941
green_kbsp$GDD[green_kbsp$year == 2016 & green_kbsp$state == "warmed"] = 1162
green_kbsp$GDD[green_kbsp$year == 2017 & green_kbsp$state == "ambient"] = 967
green_kbsp$GDD[green_kbsp$year == 2017 & green_kbsp$state == "warmed"] = 1250
green_kbsp$GDD[green_kbsp$year == 2018 & green_kbsp$state == "ambient"] = 1035
green_kbsp$GDD[green_kbsp$year == 2018 & green_kbsp$state == "warmed"] = 1261
green_kbsp$GDD[green_kbsp$year == 2019 & green_kbsp$state == "ambient"] = 965
green_kbsp$GDD[green_kbsp$year == 2019 & green_kbsp$state == "warmed"] = 1134
green_kbsp$GDD[green_kbsp$year == 2020 & green_kbsp$state == "ambient"] = 943
green_kbsp$GDD[green_kbsp$year == 2020 & green_kbsp$state == "warmed"] = 1152
green_kbsp$GDD[green_kbsp$year == 2021 & green_kbsp$state == "ambient"] = 996
green_kbsp$GDD[green_kbsp$year == 2021 & green_kbsp$state == "warmed"] = 1226
```
...
leveneTest(residuals(fitp2) ~ green_kbsp$year_factor) # doesn't work with numerical year
...
leveneTest(residuals(fitp2u) ~ green_umbsp$state)
ols_test_normality(fitp2u)
shapiro.test(resid(fitp2u))
...
mod11p <- lmer(1/(plot_half_cover_date) ~ state * as.factor(year_factor) * insecticide + (1|plot), green_kbsp, REML=FALSE)
...
1/(1.109e-02 + -7.050e-04*0) # 90.17133 warmed
1/(1.109e-02 + -7.050e-04*1) # 96.29273 ambient
# effect:
96.29273 - 90.17133 # warmed plots green-up 6.1214 days earlier than ambient
emmeans(mod5p, list(pairwise ~ state*year_factor), adjust = "tukey")
mod5p.emm <- emmeans(mod5p, ~ state * year_factor)
contrast(mod5p.emm, "consec", simple = "each", combine = F, adjust = "mvt") # for mod5p, this won't show differences between years bc there is not an interaction term in the model
emmip(mod5p, year_factor~state)
...
mod11p_u <- lmer(1/(plot_half_cover_date) ~ state * as.factor(year_factor) * insecticide + (1|plot), green_umbsp, REML=FALSE)
AICtab(modfull_u, mod1p_u, mod2p_u, mod3p_u, mod4p_u, mod5p_u, mod6p_u, mod7p_u, mod8p_u, mod9p_u, mod10p_u, mod11p_u, weights=T)
AICctab(mod7p_u, mod9p_u)
anova(mod5p_u, mod8p_u)

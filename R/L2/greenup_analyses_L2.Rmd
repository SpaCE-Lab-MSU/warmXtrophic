...
# basing cutoff for GDD at julian date 178
GDD_green <- GDD %>%
        filter(julian == 178) %>%
        select(-max_temp, -min_temp, -mean_GDD_temp, -GDD_base_10, -julian)
green_kbsp <- left_join(green_kbsp, GDD_green, by=c('year','state'))

# looking to see which temp variable best correlates with greenup dates
ggplot(green_kbsp, aes(x = GDD_cumulative, y = plot_half_cover_date, color = state))+
        geom_point() +
        geom_smooth(method = 'lm', alpha = .15, aes(fill = state)) + 
...
1/(6.988e-03 + 1.126e-03*0) # 143.1025 ambient
1/(6.988e-03 + 1.126e-03*1) # 123.2438 warmed
# effect:

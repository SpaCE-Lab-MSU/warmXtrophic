...
# final versions of these plots are in greenup_plots_L2.Rmd
...
ggplot(green_kbsp, aes(x = GDD_cumulative, y = plot_half_cover_date, color = state))+
        ...
        theme_bw()
ggplot(green_umbsp, aes(x = GDD_cumulative, y = plot_half_cover_date, color = state))+
        geom_point() +
        geom_smooth(method = 'lm', alpha = .15, aes(fill = state)) + 
        scale_color_manual(values = c("blue","red")) +
        scale_fill_manual(values = c("blue","red")) +
...
anova(mod7p, mod9p) #mod9p barely
...
mod7p.emm <- emmeans(mod7p, ~ state * year_factor)
contrast(mod7p.emm, "consec", simple = "each", combine = F, adjust = "mvt") # for mod5p, this won't show differences between years bc there is not an interaction term in the model
emmip(mod7p, year_factor~state)

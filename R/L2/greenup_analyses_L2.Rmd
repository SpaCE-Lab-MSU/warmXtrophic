...
fitp2<- lm(plot_half_cover_date~state+year, data = green_kbsp)
...
outlierTest(fitp2u) # 118 and 158 are outliers - remove? look like the only plots with a half cover date >200
...
mod1 <- lmer(spp_half_cover_date ~ state*year + insecticide*year_factor + (1|species) + (1|plot), green_kbs, REML=FALSE)
mod2 <- lmer(spp_half_cover_date ~ state*year + insecticide*year_factor + (1|species), green_kbs, REML=FALSE)
...
mod5 <- lmer(spp_half_cover_date ~ state + year_factor + insecticide + (1|species) + (1|plot), green_kbs, REML=FALSE)
anova(mod4, mod5)
...
mod6 <- lmer(spp_half_cover_date ~ state*year_factor + insecticide + (1|species) + (1+year|plot), green_kbs, REML=FALSE)
anova(mod4, mod6)
anova(mod6)
...
# these are the fixed predicted values:
plot_model(mod6, type = "pred", terms = c("year_factor", "state", "insecticide"))
# these are the random effects estimates
...
# using model 7a for overall greenup model #
...
mod9a <- lmer(spp_half_cover_date ~ state + origin + year_factor + (1|plot), green_kbs, REML=FALSE)
...
mod11a <- lmer(spp_half_cover_date ~ state + growth_habit + year_factor + (1|plot), green_kbs, REML=FALSE)
...
mod2p <- lmer(plot_half_cover_date ~ state + year_factor + (1|plot), green_kbsp, REML=FALSE)
mod3p <- lmer(plot_half_cover_date ~ state * year_factor + (1|plot), green_kbsp, REML=FALSE)
anova(mod1p, mod2p, mod3p) #mod2p
summary(mod2p)
mod2p.2 <- lmer(plot_half_cover_date ~ state + year_factor + insecticide + (1|plot), green_kbsp, REML=FALSE)
...
umod9a <- lmer(spp_half_cover_date ~ state + origin + year_factor + (1|plot), green_umbs, REML=FALSE)
...
mod2pu <- lmer(plot_half_cover_date ~ state + year_factor + (1|plot), green_umbsp, REML=FALSE)
mod3pu <- lmer(plot_half_cover_date ~ state * year_factor + (1|plot), green_umbsp, REML=FALSE)
anova(mod1pu, mod2pu, mod3pu) #mod2pu
summary(mod2pu)
outlierTest(mod2pu) # remove these outliers?
```

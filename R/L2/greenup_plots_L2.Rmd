...
#png("Spp_greenup.png", units="in", width=8, height=8, res=300)
...
#png("Plot_greenup.png", units="in", width=8, height=8, res=300)
...
greenup_overall_plot <- function(loc) { 
  greenup_plot <- subset(gr_plot, site == loc)
  return(ggplot(greenup_plot, aes(x = state, y = plot_half_cover_date)) +
           facet_grid(.~insecticide) +
           geom_boxplot(color = "black") +
           labs(x = NULL, y = NULL, title = loc) +
           #scale_fill_manual(values = c("#a6bddb", "#fb6a4a")) +
...
plot_level_overall <- ggarrange(plot_overall_kbs, plot_overall_umbs, nrow = 2, common.legend = T, legend="right")
#png("Plot_greenup_overall.png", units="in", width=8, height=8, res=300)
...
## Note: half/median date used for greenup and flower, but min date used for seed set (need to fix this)
```{r, echo=F}
# green-up plots
...
flwr_overall_plot <- function(loc) { 
  ...
  return(ggplot(flower_plot, aes(x = state, y = julian_median)) +
           facet_grid(.~insecticide) +
           geom_boxplot(color = "black") +
           labs(x = NULL, y = "Flower") +
           #scale_fill_manual(values = c("#a6bddb", "#fb6a4a")) +
           #scale_x_discrete(labels=c("ambient" = "A", "warmed" = "W")) +
           theme_classic())
}
...
flwr_dur_plot <- function(loc) { 
  flower_plot <- subset(flwr_plot, site == loc)
  return(ggplot(flower_plot, aes(x = state, y = flwr_duration)) +
           facet_grid(.~insecticide) +
           geom_boxplot(color = "black") +
...
# seed set plots - need to get median date for this, using min date for now
sd_overall_plot <- function(loc) { 
  seed_plot <- subset(sd_plot, site == loc)
  return(ggplot(seed_plot, aes(x = state, y = julian_min)) +
           facet_grid(.~insecticide) +
           geom_boxplot(color = "black") +
...
phen_overall <- ggarrange(plot_overall_kbs2, plot_overall_umbs2,
                          flwr_overall_kbs, flwr_overall_umbs,
                          sd_overall_kbs, sd_overall_umbs,
                          nrow = 3, ncol = 2, common.legend = T, legend="right")
#png("All_phenology.png", units="in", width=8, height=8, res=300)

...
```{r}
# greenup
sum_green_plot_i2$year <- as.factor(sum_green_plot_i2$year)
sum_green_plot_i2$full_treat <- paste(sum_green_plot_i2$state, sum_green_plot_i2$insecticide, sep="_")
gr_line_i <- function(loc) { 
        gr_plot <- subset(sum_green_plot_i2, site == loc)
        return(ggplot(gr_plot, aes(x = year, y = avg_julian, group=full_treat, linetype=full_treat, color = full_treat)) +
                       geom_errorbar(aes(ymin=avg_julian-se, ymax=avg_julian+se), width=.1,color="black",linetype="solid") +
                       geom_line(size = 1) +
                       geom_point(size = 2) +

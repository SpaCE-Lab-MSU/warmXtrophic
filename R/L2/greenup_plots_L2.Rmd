...
## Combining greenup, flower, and seed into one figure - plot level
## bar plots with insecticide
```{r}
# greenup
sum_green_plot_i3_v2$full_treat <- paste(sum_green_plot_i3_v2$state, sum_green_plot_i3_v2$insecticide, sep="_")
gr_bar_i <- function(loc) { 
        gr_plot <- subset(sum_green_plot_i3_v2, site == loc)
        return(ggplot(gr_plot, aes(x = full_treat, y = avg_julian, group=full_treat,fill = full_treat)) +
                       geom_bar(stat = "identity",position="dodge") +
                       geom_errorbar(aes(ymin=avg_julian-se, ymax=avg_julian+se), color="black",linetype="solid",width=.3,position=position_dodge(0.1)) +
...
gr_bar_i_kbs <- gr_bar_i("KBS")
gr_bar_i_kbs <- gr_bar_i_kbs + theme(axis.text.x=element_blank(),axis.ticks.x = element_blank())
gr_bar_i_umbs <- gr_bar_i("UMBS")
gr_bar_i_umbs <- gr_bar_i_umbs + labs(y=NULL) + theme(axis.text.x=element_blank(),
                                                        axis.title.y=element_blank(),
                                                      axis.ticks.x = element_blank())
                                                        #axis.text.y=element_blank())
#flower
sum_flwr_plot_i2$full_treat <- paste(sum_flwr_plot_i2$state, sum_flwr_plot_i2$insecticide, sep="_")
flwr_bar_i <- function(loc) { 
        flwr_plot <- subset(sum_flwr_plot_i2, site == loc)
        return(ggplot(flwr_plot, aes(x = full_treat, y = avg_julian, group=full_treat, fill = full_treat)) +
                       geom_bar(stat="identity",position="dodge") +
                       geom_errorbar(aes(ymin=avg_julian-se, ymax=avg_julian+se), color="black",linetype="solid",position=position_dodge(0.1), width=.3) +
                       scale_fill_manual(name="Treatment",
                                          values = c("#a6bddb", "#687689", "#fb6a4a", "#9D422E"),
...
flwr_bar_i_kbs <- flwr_bar_i("KBS")
flwr_bar_i_kbs <- flwr_bar_i_kbs + theme(axis.text.x=element_blank(),axis.ticks.x = element_blank()) + labs(title=NULL)
flwr_bar_i_umbs <- flwr_bar_i("UMBS")
flwr_bar_i_umbs <- flwr_bar_i_umbs + labs(y=NULL, title=NULL) + theme(axis.text.x=element_blank(),
                                                        axis.title.y=element_blank(),
                                                        axis.ticks.x = element_blank())
                                                        #axis.text.y=element_blank())
...
sd_bar_i <- function(loc) { 
        sd_plot <- subset(sum_sd_plot_i2, site == loc)
        return(ggplot(sd_plot, aes(x = full_treat, y = avg_julian, group=full_treat, fill = full_treat)) +
                       geom_bar(stat="identity",position="dodge") +
                       geom_errorbar(aes(ymin=avg_julian-se, ymax=avg_julian+se), color="black",linetype="solid",position=position_dodge(0.1), width=.3) +
                       scale_fill_manual(name="Treatment",
                                          values = c("#a6bddb", "#687689", "#fb6a4a", "#9D422E"),
...
sd_bar_i_kbs <- sd_bar_i("KBS")
sd_bar_i_kbs <- sd_bar_i_kbs + labs(title=NULL)+ theme(axis.text.x=element_blank(),axis.ticks.x = element_blank())
sd_bar_i_umbs <- sd_bar_i("UMBS")
sd_bar_i_umbs <- sd_bar_i_umbs + labs(y=NULL, title=NULL) + theme(axis.text.x=element_blank(),
                                                        axis.title.y=element_blank(),
                                                        axis.ticks.x = element_blank())
                                                        #axis.text.y=element_blank())

phen_bar_i <- ggpubr::ggarrange(gr_bar_i_kbs, gr_bar_i_umbs,
                          flwr_bar_i_kbs, flwr_bar_i_umbs,
                          sd_bar_i_kbs, sd_bar_i_umbs,
                          nrow = 3, ncol = 2, common.legend = T, legend="right")
png("greenup_plots_L2_all_phenology_bar_insect.png", units="in", width=10, height=8, res=300)
annotate_figure(phen_bar_i,
                left = text_grob("Phenological event julian date", color = "black", rot = 90, size=15))
dev.off()
```

## Combining greenup, flower, and seed into one figure - plot level
## dot plots with insecticide
```{r}
# greenup
sum_green_plot_i3_v2$full_treat <- paste(sum_green_plot_i3_v2$state, sum_green_plot_i3_v2$insecticide, sep="_")
gr_dot_i <- function(loc) { 
        gr_plot <- subset(sum_green_plot_i3_v2, site == loc)
        return(ggplot(gr_plot, aes(x = state, y = avg_julian, shape = insecticide)) +
                       #geom_point(stat = "identity",position=position_dodge(0.2),size=5) +
                       geom_pointrange(aes(ymin=avg_julian-se, ymax=avg_julian+se), color="black",linetype="solid",size=1,position=position_dodge(0.2)) +
...
flwr_dot_i <- function(loc) { 
        flwr_plot <- subset(sum_flwr_plot_i2, site == loc)
        return(ggplot(flwr_plot, aes(x = state, y = avg_julian, shape = insecticide)) +
                       #geom_point(stat = "identity",position=position_dodge(0.2),size=5) +
                       geom_pointrange(aes(ymin=avg_julian-se, ymax=avg_julian+se), color="black",linetype="solid",size=1,position=position_dodge(0.2)) +
                       scale_shape_manual(name="Treatment",
                                          values = c(1, 19),
...
flwr_dot_i_kbs <- flwr_dot_i_kbs + labs(title=NULL)
...
sd_dot_i <- function(loc) { 
        sd_plot <- subset(sum_sd_plot_i2, site == loc)
        return(ggplot(sd_plot, aes(x = state, y = avg_julian, shape = insecticide)) +
                       #geom_point(stat = "identity",position=position_dodge(0.2),size=5) +
                       geom_pointrange(aes(ymin=avg_julian-se, ymax=avg_julian+se), color="black",linetype="solid",size=1,position=position_dodge(0.2)) +
                       scale_shape_manual(name="Treatment",
                                          values = c(1, 19),
...
sd_dot_i_kbs <- sd_dot_i_kbs + labs(title=NULL)
...
greenup_plot_overall <- function(loc) { 
  ...
  return(ggplot(greenup_spp, aes(x = full_treat, y = avg_julian, fill = full_treat)) +

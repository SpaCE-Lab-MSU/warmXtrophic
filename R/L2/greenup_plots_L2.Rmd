...
# fixing "21" year for flwr and seed plot data
flwr_plot$year[flwr_plot$year == 21] <- 2021
sd_plot$year[sd_plot$year == 21] <- 2021
...
```{r}
# greenup
sum_green_plot$year <- as.factor(sum_green_plot$year)
gr_line <- function(loc) { 
  gr_plot <- subset(sum_green_plot, site == loc)
  return(ggplot(gr_plot, aes(x = year, y = avg_julian, group = state)) +
                 geom_errorbar(aes(ymin=avg_julian-se, ymax=avg_julian+se), width=.1) +
...
sd_line_umbs <- sd_line_umbs + labs(y=NULL, title=NULL)
...
### note to self: left off here, go back up & re-summarize data with insecticide included
```{r}
# greenup
sum_green_plot_i$year <- as.factor(sum_green_plot_i$year)
sum_green_plot_i$full_treat <- paste(sum_green_plot_i$state, sum_green_plot_i$insecticide, sep="_")
gr_line_i <- function(loc) { 
        gr_plot <- subset(sum_green_plot_i, site == loc)
...
sd_line_i_umbs <- sd_line_i_umbs + labs(y=NULL, title=NULL)

phen_line_i <- ggpubr::ggarrange(gr_line_i_kbs, gr_line_i_umbs,
                          flwr_line_i_kbs, flwr_line_i_umbs,
                          sd_line_i_kbs, sd_line_i_umbs,
                          nrow = 3, ncol = 2, common.legend = T, legend="right")
png("greenup_plots_L2_all_phenology_line_insect.png", units="in", width=11, height=8, res=300)

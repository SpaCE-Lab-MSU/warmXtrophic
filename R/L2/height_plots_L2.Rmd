...
```{r}
# plot level KBS
kbs_height_dot <- ggplot(kbs_height_sum, aes(x = state, y = avg_height, fill = state)) +
...
# plot level UMBS
umbs_height_dot <- ggplot(umbs_height_sum, aes(x = state, y = avg_height, fill = state)) +
        geom_pointrange(aes(ymin=avg_height-se, ymax=avg_height+se), pch=21,size=1,position=position_dodge(0.2)) +
        scale_fill_manual(name="Treatment",
                          values = c("#a6bddb", "#fb6a4a")) +

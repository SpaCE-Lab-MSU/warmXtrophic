...
umbs_comp_plot_amb <- umbs_comp_plot %>%
        filter(state == "ambient")
modtest1u <- lmer(log(avg_pc) ~ mean_temp + (1|plot), kbs_comp_plot_amb, REML=FALSE)
modtest2u <- lmer(log(avg_pc) ~ GDD_cumulative + (1|plot), kbs_comp_plot_amb, REML=FALSE)
...
# comparisons
emmeans(mod_exotic_k, list(pairwise ~ state*insecticide), adjust = "tukey")
mod_exotic_k.emm <- emmeans(mod_exotic_k, ~ state*insecticide)
contrast(mod_exotic_k.emm, "consec", simple = "each", combine = F, adjust = "mvt")
emmip(mod_exotic_k, insecticide~state)

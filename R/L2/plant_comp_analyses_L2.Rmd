...
# note: the species data frame is only used to demonstrate that plant differs between species (for the supplement)
comp_spp <- read.csv(file.path(L2_dir, "plant_composition/final_plant_comp_species_L2.csv")) 
comp_spp$relative <- as.numeric(as.integer(comp_spp$relative))

comp_origin_spp <- read.csv(file.path(L2_dir, "plant_composition/final_plant_comp_origin_species_L2.csv")) 
comp_origin_spp$relative <- as.numeric(as.integer(comp_origin_spp$relative))

comp_growth_spp <- read.csv(file.path(L2_dir, "plant_composition/final_plant_comp_growth_habit_species_L2.csv")) 
comp_growth_spp$relative <- as.numeric(as.integer(comp_growth_spp$relative))
```
...
# effect:
0.01797704 - 0.01553111 # effect of warming - warmed plots had 0.00244593 or 0.24% more relative abundance than ambient plots
```
...
```{r}
# UMBS PLOT LEVEL
modfullu <- lmer(log(avg_relative_abundance) ~ state*insecticide + year_factor + (1|plot), umbs_comp_plot, REML = FALSE)
...
# effect:
0.04611071 - 0.0419071 # 0.00420361
```
...
mod_spp_k <- lmer(relative ~ state * insecticide + species + as.factor(year_factor) + (1|plot), comp_kbs_spp, REML=FALSE)
anova(mod_spp_k)
# making a table
...
# UMBS
# species model for supp
mod_spp_u <- lmer(julian_min ~ state * insecticide + species + as.factor(year_factor) + (1|plot), comp_umbs_spp, REML=FALSE)
anova(mod_spp_u)
# making a table

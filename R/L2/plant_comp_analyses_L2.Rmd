...
exp((-1.98820) +  (-0.228)*0) # 0.1369417 
exp((-1.98820) +  (-0.228)*1) # 0.1090226
# effect:
0.1090226 - 0.1369417 # effect of warming - warmed plots had 0.0279191 or 2.79191% more relative abundance than ambient plots
...
modtest1u <- lmer(log(avg_relative_abundance) ~ mean_temp + (1|plot), kbs_comp_plot, REML=FALSE)
modtest2u <- lmer(log(avg_relative_abundance) ~ GDD_cumulative + (1|plot), kbs_comp_plot, REML=FALSE)
...
# calculating effect size - accounting for log transformation
exp((-4.16491) + (0.14625)*0) # 0.01553111 
exp((-4.16491) + (0.14625)*1) # 0.01797704
# effect:
0.01797704 - 0.01553111 # effect of warming - warmed plots had 0.0279191 or 2.79191% more relative abundance than ambient plots
```
...
fit3 <- lm(sqrt(avg_relative_abundance) ~ state * year, data = kbs_origin_plot)
...
#### Note to self - need to work on the model structure below
# hypothesized model
mod_k_o <- lmer(sqrt(avg_relative_abundance) ~ state * insecticide + origin + year_factor + (1|plot), kbs_origin_plot, REML=FALSE)
...
fit3 <- lm(log(avg_relative_abundance) ~ state * year, data = umbs_comp_plot)
interact_plot(fit3, pred = year, modx = state) 
```
...
# note: including state, year, and temp data into a model leads to rank deficiency
# so below, we test for avg relative abundance as a function of just temp to see how real temp data affects avg relative abundance
modtest1u <- lmer(log(avg_relative_abundance) ~ mean_temp + (1|plot), umbs_comp_plot, REML=FALSE)
modtest2u <- lmer(log(avg_relative_abundance) ~ GDD_cumulative + (1|plot), umbs_comp_plot, REML=FALSE)
...
# calculating effect size - accounting for log transformation
exp((-3.17230) + (0.09559)*0) # 0.0419071 
exp((-3.17230) + (0.09559)*1) # 0.04611071
# effect:
0.04611071 - 0.0419071
```

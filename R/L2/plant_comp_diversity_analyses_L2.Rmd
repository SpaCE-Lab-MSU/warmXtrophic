...
comp_kbs <- subset(peak_comp, site == "kbs") %>% dplyr::select(plot, species, cover, year)
comp_umbs <- subset(peak_comp, site == "umbs") %>% dplyr::select(plot, species, cover, year)
...
outlierTest(fit_state_kbs) # yes row 60 (2017), 152 (2021), 148 (2021)
...
outlierTest(fit_state_umbs) # yes, row 178, 190
...
outlierTest(mod1) # row 60 and 152
...
mod1 <- lmer(log(simpson) ~ state*year_factor + insecticide*year_factor + (1|plot), kbs_diversity, REML = FALSE)
mod2 <- lmer(log(simpson) ~ state*year_factor + insecticide + year_factor + (1|plot), kbs_diversity, REML=FALSE)
...
mod4 <- lmer(log(simpson) ~ state + year + insecticide + (1|plot), kbs_diversity, REML=FALSE)
anova(mod3, mod4) # stick with mod3
AICctab(mod3, mod4, weights=T) # mod3
...
# table <- anova(mod10, mod9)
# kable(table) %>% kableExtra::kable_styling()
# AICctab(mod10pu, mod9pu, weights=T)
...
outlierTest(mod1u) # yes outliers
...
summ(mod2u)
summary(mod2u)
emmeans(mod2u, list(pairwise ~ state*year), adjust = "tukey")
```
...
summary(mod1us)
summary(mod2us)
AICctab(mod1us, mod2us, weights=T) # model 2

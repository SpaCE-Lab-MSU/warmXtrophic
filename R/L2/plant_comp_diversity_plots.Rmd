...
DATA INPUT: Plant composition diversity from warmX Google drive L2 folder
...
simpson_overall_plot <- function(loc) { 
  ...
  return(ggplot(simpson_plot, aes(x = state, y = simpson, fill=state)) +
           ...
           geom_boxplot(outlier.shape=NA, alpha=0.7) +
           geom_jitter(aes(alpha=0.8, color=state, fill=state), shape=16, size=2) +
           labs(x = NULL, y = "Simpson Diversity", title = loc) +
           scale_color_manual(values = c("#a6bddb", "#fb6a4a")) +
           scale_fill_manual(values = c("#a6bddb", "#fb6a4a")) +
           scale_x_discrete(labels=c("ambient" = "A", "warmed" = "W")) +
           theme_classic()) +
...
png("diversity_plots_L2_all_diversity_measures_boxplot.png", units="in", width=8, height=8, res=300)
annotate_figure(diversity_overall_boxplot,
                left = text_grob("Diversity", color = "black", rot = 90),
                bottom = text_grob("Treatment", color = "black"))
dev.off()
```

```{r, echo=F}
# how to make the desired plot:
...
simpson_overall_plot <- function(loc) { 
  ...
  return(ggplot(simpson_plot, aes(x = year, y = avg_simpson, group = state)) +
           ...
           geom_line(aes(color=state), size = 1) +
           geom_point(aes(color=state), size = 2) +
           labs(x = NULL, y = "Simpson's Diversity", title = loc) +
           theme_classic())
}
...
png("diversity_plots_L2_all_diversity_measures_lineplot.png", units="in", width=8, height=8, res=300)

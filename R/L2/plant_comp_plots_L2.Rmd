...
comp_all_dot_i <- ggpubr::ggarrange(comp_dot_i_kbs, comp_dot_i_umbs,
                                ...
                                widths = c(1.1, 0.9))
png("percent_cover_plots_L2_all_plant_comp_dot_insect.png", units="in", width=8, height=6, res=300)
annotate_figure(comp_all_dot_i,
                left = text_grob("Cover (%)", color = "black", rot = 90, size=17))
dev.off()
```

The plots below have not been updated like the one above
Origin & Growth Habit
```{r}
# origin
...
origin_dot_i_kbs <- origin_dot_i_kbs + 
        annotate("text", x = 0.5, y=0.7, label = "A", size=5) +
...
origin_dot_i_umbs <- origin_dot_i_umbs + 
        labs(y=NULL) + 
        annotate("text", x = 0.5, y=0.7, label = "B", size=5) +
...
growth_dot_i_kbs <- growth_dot_i_kbs + 
        annotate("text", x = 0.5, y=0.7, label = "C", size=5) +
        theme(plot.title = element_text(size=20),
              axis.text.y = element_text(size=17),
              axis.title.y = element_text(size=17),
              legend.position = "none")
...
growth_dot_i_umbs <- growth_dot_i_umbs + 
        labs(y=NULL) + 
        annotate("text", x = 0.5, y=0.7, label = "D", size=5) +
        theme(axis.title.y=element_blank(),
              plot.title = element_text(size=20),
              axis.text.y = element_blank(),
              legend.text = element_text(size=17),
...
png("relative_abundance_plots_L2_growth_origin_dot_insect.png", units="in", width=10, height=8, res=300)
...
comp_plot_overall <- function(loc) { 
        comp_spp <- subset(sum_comp_spp_i2, site == loc)
        return(ggplot(comp_spp, aes(x = state, y = avg_relative, fill = insecticide)) +
                       facet_wrap(~species, ncol=4,scales="free") +
                       geom_pointrange(aes(ymin=avg_relative-se, ymax=avg_relative+se), pch=21,size=0.6,position=position_dodge(0.3)) +
                       scale_fill_manual(name="Treatment",
                                         values = c("#FFB451", "#0b0055"),
                                         labels=c("Herbivory","Reduced Herbivory")) +
...
png("comp_species.png", units="in", width=21, height=15, res=300)

...
origin_dot_i <- function(loc) { 
       origin_plot <- subset(sum_origin_plot_i, site == loc)
        return(ggplot(origin_plot, aes(x = state, y = avg_relabun, fill=insecticide)) +
                       #geom_point(stat = "identity",position=position_dodge(0.2),size=5) +
                       geom_pointrange(aes(ymin=avg_relabun-se, ymax=avg_relabun+se), pch=21,size=1,position=position_dodge(0.3)) +
                       ...
                       scale_fill_manual(name="Treatment",
                                         values = c("#FFB451", "#0b0055"),
                                         labels=c("Herbivory","Reduced Herbivory")) +
                               labs(x = NULL, y = "Native vs. Exotic", title = loc) +
                       scale_x_discrete(labels=c("ambient" = "Ambient", "warmed" = "Warmed")) +
...
origin_dot_i_kbs
origin_dot_i_kbs <- origin_dot_i_kbs + labs(title=NULL) + theme(axis.text.x = element_blank(),
                                                            axis.text.y = element_text(size=17),
                                                            axis.title.y = element_text(size=17),
                                                            legend.text = element_text(size=17),
                                                            legend.title = element_text(size=17))
origin_dot_i_umbs <- origin_dot_i("UMBS")
origin_dot_i_umbs <- origin_dot_i_umbs + labs(y=NULL, title=NULL) + theme(axis.title.y=element_blank(),
                                                                      axis.text.y = element_blank(),
...
growth_dot_i <- function(loc) { 
       growth_plot <- subset(sum_growth_plot_i, site == loc)
        return(ggplot(growth_plot, aes(x = state, y = avg_relabun, fill=insecticide)) +
                       #geom_point(stat = "identity",position=position_dodge(0.2),size=5) +
                       geom_pointrange(aes(ymin=avg_relabun-se, ymax=avg_relabun+se), pch=21,size=1,position=position_dodge(0.3)) +
                       #scale_shape_manual(name="Treatment",
                       #                   values = c(1, 19),
...
growth_dot_i_kbs <- growth_dot_i_kbs + labs(title=NULL) + theme(axis.text.y = element_text(size=17),
                                                        axis.text.x = element_text(size=17),
                                                        axis.title.y = element_text(size=17),
                                                        legend.text = element_text(size=17),
                                                        legend.title = element_text(size=17))
growth_dot_i_umbs <- growth_dot_i("UMBS")
growth_dot_i_umbs <- growth_dot_i_umbs + labs(y=NULL, title=NULL) + theme(axis.title.y=element_blank(),
                                                                  axis.text.y = element_blank(),
                                                                  axis.text.x = element_text(size=17),
                                                                  legend.text = element_text(size=17),
                                                                  legend.title = element_text(size=17))
...
comp_all_dot_i <- ggpubr::ggarrange(comp_dot_i_kbs, comp_dot_i_umbs,
                                origin_dot_i_kbs, origin_dot_i_umbs,
                                growth_dot_i_kbs, growth_dot_i_umbs,
                                nrow = 3, ncol = 2, common.legend = T, legend="right",
                                widths = c(1.1, 0.9,1.1,0.9,1.1,0.9))
png("relative_abundance_plots_L2_all_plant_comp_dot_insect.png", units="in", width=10, height=10, res=300)
annotate_figure(comp_all_dot_i,
                left = text_grob("Relative Abundance", color = "black", rot = 90, size=17))
dev.off()
```

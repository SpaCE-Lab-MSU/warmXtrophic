...
pc_avg <- comp %>%
  ...
  summarize(avg_pc = mean(avg_pc, na.rm = TRUE),
            se = std.error(avg_pc, na.rm = TRUE))
...
names(de)<-c("site","state", "year", "avg_pc", "se")
...
names(de)<-c("site","state", "origin", "year", "avg_pc", "se")
names(de2)<-c("site","state", "origin", "year", "avg_pc", "se")
de$year <- as.integer(de$year)
...
names(de)<-c("site","state", "growth_habit", "year", "avg_pc", "se")
names(de2)<-c("site","state", "growth_habit", "year", "avg_pc", "se")
de$year <- as.integer(de$year)
de2$year <- as.integer(de2$year)
...
plot_line <- function(loc) { 
  ...
  return(ggplot(all_plot, aes(x = year, y = avg_pc, group = state)) +
...
gdd_comp_merge <- ggpubr::ggarrange(kbs_comp_gdd,umbs_comp_gdd,
                                    kbs_comp_mean,umbs_comp_mean,
                                    kbs_comp_med,umbs_comp_med,
                                    kbs_comp_max,umbs_comp_max,
                                    ncol = 2, nrow=4,common.legend=T,legend="none")
png("comp_gdd_temp.png", units="in", width=7, height=8, res=300)

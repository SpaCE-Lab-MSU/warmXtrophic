...
#kbs_sd_plot$log_julian_min <- log(kbs_sd_plot$julian_min)
#hist(kbs_sd_plot$log_julian_min)
#shapiro.test(kbs_sd_plot$log_julian_min) # this says the data isn't normal so not sure if I should proceed with log #transformation going forward or not
```
...
# Exploring distributions for these right-skewed data:
descdist(umbs_sd_spp$julian_min, discrete = FALSE)
...
gofstat(list(fit.gamma, fit.weibull, fit.ln), fitnames = c("Gamma", "Weibull", "Log Normal"))
# Lognormal is slightly better than gamma - going with a lognormal transformation for umbs seed set at the species level

#umbs_sd_spp$log_julian_min <- log(umbs_sd_spp$julian_min)
#hist(umbs_sd_spp$log_julian_min)
#shapiro.test(umbs_sd_spp$log_julian_min) # this says the data isn't normal so not sure if I should proceed with log #transformation going forward or not
```
...
shapiro.test(umbs_sd_plot$julian_min) # pvalue is < 0.05 so we reject the null hypothesis that the data is normal (aka not normally distributed)
...
# Exploring distributions for these right-skewed data:
descdist(umbs_sd_plot$julian_min, discrete = FALSE)
...
gofstat(list(fit.gamma, fit.weibull, fit.ln), fitnames = c("Gamma", "Weibull", "Log Normal"))
# Gamma is slightly better here but going to go with a lognormal transformation for umbs seed set at the plot level

#umbs_sd_plot$log_julian_min <- log(umbs_sd_plot$julian_min)
#hist(umbs_sd_plot$log_julian_min)
#shapiro.test(umbs_sd_plot$log_julian_min) # this says the data isn't normal so not sure if I should proceed with log #transformation going forward or not
```
...
outlierTest(fit_spp_stateyear_kbs) 
...
outlierTest(fit_spp_stateyear_umbs)
...
# Plot level data
# UMBS State-only model
fit_plot_state_umbs <- lm(log(julian_min) ~ state, data = umbs_sd_plot)
...
# UMBS State and year model
fit_plot_stateyear_umbs <- lm(log(julian_min) ~ state + year_factor, data = umbs_sd_plot)
outlierTest(fit_spp_stateyear_umbs) 
...
fit3_plot_umbs <- lm(log(julian_min) ~ state * year_factor, data = umbs_sd_plot)
...
anova(mod1, mod3) # Don't need insecticide, continue with mod3
...
mod3 <- lmer(log(julian_min) ~ state*year_factor + (1|species), kbs_sd_spp, REML=FALSE)
mod4 <- lmer(log(julian_min) ~ state*year_factor + insecticide + (1|species) + (1|plot), kbs_sd_spp, REML=FALSE)
mod5 <- lmer(log(julian_min) ~ state + year_factor + (1|species), umbs_flwr_spp, REML=FALSE)
mod6 <- lmer(log(julian_min) ~ state + year_factor + (1|species) + (1 + year|plot), kbs_sd_spp, REML=FALSE)

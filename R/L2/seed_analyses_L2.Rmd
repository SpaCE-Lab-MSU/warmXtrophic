...
# KBS State-only model
fit_plot_state_kbs <- lm(log(julian_min) ~ state, data = kbs_sd_plot_nooutlier)
outlierTest(fit_plot_state_kbs) # outliers - 144, 217
qqPlot(fit_plot_state_kbs, main="QQ Plot") 
hist(fit_plot_state_kbs$residuals)
leveragePlots(fit_plot_state_kbs)
...
mod_k_all <- lmer(log(julian_min) ~ state * insecticide * year_factor + (1|plot), kbs_sd_plot_nooutlier, REML=FALSE)
summary(mod_k_all)
anova(mod_k_all)

# calculating effect size - accounting for log transformation
exp((5.341e+00) +  (7.018e-02)*0) # 208.7213 
exp((5.341e+00) +  (7.018e-02)*1) # 223.8956
# effect:
223.8956 - 208.7213 # = 15.1743  
# warming plots delayed seed set by 15.2 days 

# comparisons
contrast(emmeans(mod_k_all, ~state*year_factor), "pairwise", simple = "each", combine = F, adjust = "mvt")
# making a table
kable(anova(mod_k_all), digits = 2) %>% kableExtra::kable_styling()
...
exp((4.984e+00) + (2.009e-02)*1) # 149.0214
# effect:
149.0214 - 146.0574 # = 2.965  
# warming plots delayed seed set by ~2.965 days
...
# comparisons
contrast(emmeans(mod_u_all, ~state*insecticide*year_factor), "pairwise", simple = "each", combine = F, adjust = "mvt")
# making a table
kable(anova(mod_u_all), digits = 3) %>% kableExtra::kable_styling()
...
anova(mod_spp_u)
# making a table
kable(anova(mod_spp_u), digits = 3) %>% kableExtra::kable_styling()
```

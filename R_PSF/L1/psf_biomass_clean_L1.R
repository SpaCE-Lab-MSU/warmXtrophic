# TITLE: Plant Biomass - PSF experiment 2021
# AUTHORS: Moriah Young
# COLLABORATORS: Kara Dobson
# DATA INPUT: raw data - psf_biomass.csv
# DATA OUTPUT: cleaned data - psf_biomass_L1.csv
...
biomass <- read.csv(file.path(L0_dir, "psf_biomass_2021.csv"))

names(meta)[1] <- "plant_ID" #changing column name
biomass_1 <- merge(meta, biomass, all = TRUE) # merge meta data with biomass data
biomass_2 <- na.omit(biomass_1)

# write a new csv with the cleaned and merge data and upload to the shared google drive L1 folder
write.csv(biomass_2, file.path(L1_dir, "./psf_biomass_2021_L1.csv"), row.names=F)

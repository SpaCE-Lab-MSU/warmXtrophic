...
#KBS
k<-read.csv("C:/Users/Garrett/Desktop/Ki_Oecologia_Paper/KBS_2015_Plantcomp.csv", strip.white=T, na.strings="na", header = TRUE)
summary(k)
...
spinfo<-read.csv("C:/Users/Garrett/Desktop/Ki_Oecologia_Paper/KBS_Species_Traits_Comp_15.csv")
str(spinfo)
...
kcover2$site <- "kbs"
kcover2 <- subset(kcover2, species!= 'Posp')
kcover2 <- subset(kcover2, species!= 'Pesp')
kcover2 <- subset(kcover2, species!= 'Prsp')
kcover2 <- subset(kcover2, species!= 'Thar')
...
moda <- lmer(lncov ~ mammals + (1|species), kcover2, REML=FALSE)
modb <- lmer(lncov ~ (1|species), kcover2, REML=FALSE)
anova(modb, moda)

#F-test results for warmed v. ambient plots
#ensure no missing sp.info values
mod5<-lmer(lncov~ origin + (1|species), kcover2)
anova(mod5, mod4)
summary(mod5)
anova(mod5)
...
############################################# UMBS #############################################
u<-read.csv("C:/Users/Garrett/Desktop/Ki_Oecologia_Paper/UMBS_2015_Plantcomp.csv", strip.white=T, na.strings="na", header = TRUE)
summary(u)
...
#merge in species info
spinfo<-read.csv("C:/Users/Garrett/Desktop/Ki_Oecologia_Paper/UMBS_Species_Traits_Comp_15.csv")
str(spinfo)
...
ucover2$site <- "umbs"
ucover2 <- subset(ucover2, species!= 'Prpe')
...
mod5<- lmer(lncov~ origin + (1|species), ucover2)
anova(mod5, mod4)

#Test for Phoebe Shock Experiment
moda <- lmer(lncov ~ mammals + (1|species), ucover2, REML=FALSE)
modb <- lmer(lncov ~ (1|species), ucover2, REML=FALSE)
anova(modb, moda)
...
write.csv(combinedsites, file = "combined_sites_spcomp15.csv")
...
cov<-read.csv("~/Papers/Ch2_Herbivory_Paper/Data/combined_sites_spcomp15.csv", strip.white=T, na.strings="na", header = TRUE)

...
```{r, eval = F}
# not evaluated because it crashes when knitting - working on a fix
# note to self - re-organize greennup csv to remove species influence for this part (merge by plot)
# for now, remove duplicates in plot-level dataframe by referencing unique combinations of first 3 columns (plot_half_cover_date,plot,year):
green_kbsp<-green_kbs[!duplicated(green_kbs[1:3]),]
...
green_kbsp<-subset(green_kbsp,select=c("plot_half_cover_date","plot","year","state","insecticide","site","year1"))
head(green_kbsp)
...
p <- ggplot(data = green_kbsp, aes(x = plot_half_cover_date, fill=state)) + geom_histogram(alpha=0.5, binwidth=10)
p + facet_wrap(~year1)

# Density plot
p <- ggplot(data = green_kbsp, aes(x = plot_half_cover_date, fill=state)) + geom_density(alpha=0.5)
p + facet_wrap(~year1)
...
```{r, eval = F}
# also not evaluating because this chunk crashes when knitting as well - working on it
...
outlierTest(fit)
# record 2739 is the largest outlier
...
fit5 <- lm(plot_half_cover_date ~ state + year + species, data = green_kbs)
interact_plot(fit5, pred = year, modx = state)
fit6 <- lm(plot_half_cover_date ~ state * year + species, data = green_kbs)
...
anova(mod1, mod3)
# Looks like yes, insecticide improves model fit so we will continue to include it and stick with mod1

# Does year need to be interactive with insecticide?
mod4 <- lmer(spp_half_cover_date ~ state*year + insecticide + year + (1|species) + (1|plot), green_kbs, REML=FALSE)
anova(mod1, mod4)
# Yes, insecticide*year improves model fit so we will continue to include it and stick with mod1

# Does year need to be interactive with state?
mod5 <- lmer(spp_half_cover_date ~ state + year + insecticide*year + (1|species) + (1|plot), green_kbs, REML=FALSE)
anova(mod1, mod5)
# No, P>0.05 so state*year doesn't improve model fit so we will drop it and go with mod5
summary(mod5)
anova(mod5)
...
# these are the fixed predicted values:
plot_model(mod5, type = "pred", terms = c("year", "state", "insecticide"))
# these are the random effects estimates
plot_model(mod5, type = "re", terms = c("species", "plot"))

# Let's try dropping insecticide*year
mod6 <- lmer(spp_half_cover_date ~ state + year + insecticide + (1|species) + (1|plot), green_kbs, REML=FALSE)
anova(mod5, mod6)
# Yes, insecticide*year still improves model fit so we will continue to include it and stick with mod5
summary(mod6)
anova(mod6)
...
# mod5 (and mod7) are pretty complex. We could consider an alternative model that's simpler and also one that provides more insight about the species, even if it's not quite as good a fit to the data. That would be something like this:
mod8 <- lmer(spp_half_cover_date ~ state + year + species + (1|plot), green_kbs, REML=FALSE)
anova(mod5, mod8)
summary(mod8)
plot_model(mod8, sort.est = TRUE)
# these are the fixed predicted values: - removed species from this (for now) to see the overall picture
...
# Not quite working yet: commented out for now so the Rmd can knit

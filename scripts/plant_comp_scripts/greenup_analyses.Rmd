...
# Next, plot the model. There are multiple variables but here's one way to do it based on this post:
# https://stats.stackexchange.com/questions/98958/plots-to-illustrate-results-of-linear-mixed-effect-model
newdat <- expand.grid(Sex=unique(green_kbs$state),
                  age=c(min(green_kbs$year),
                            max(green_kbs$year)))
p <- ggplot(green_kbs, aes(x=year, y=spp_half_cover_date, colour=state)) +
  geom_point(size=3) +

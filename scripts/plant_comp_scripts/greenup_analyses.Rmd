...
NOTES: [update to incorporate changes for species, and plot-level greenness when complete]
"half_cover_date" is the date at which 50% of a species max cover was reached (per species, per plot, per year)
Each row in "greenup" is the date at which half_cover_date was recorded, per species 
...
We should also think about how we're treating year. The formulation below also has a state * year interaction as a fixed effect, which means that the warming or ambient treatment could affect the half_cover_date differently in each year (there would be a different slope each year in the relationship between half_cover_date (y) and year (x), depending on state. If we just had state + year, the treatment and year would independently impact the half_cover_date.

```{r}
# Interaction plot (ignore for now the repeated measures with species):
fit1 <- lm(half_cover_date ~ state * year, data = green_kbs)
